...
`m1_rollout.py` 解决“20 个独立 case 不能冒充在线记忆连续运行”以及“候选不能由 reference 标签临时拼出来”的问题：输入是当前 versioned world、当前证据标识、固定种子和 train/validation split，输出是 20-decision 世界链，以及每步固定 16 个、真实执行、canonical state 去重后的事务候选；paired mode 还在每组放入一个“当前完全相同、合法 reference 不同”的 ambiguity pivot。例如生成器无论隐藏标签是 BIRTH 还是 RELINK，都会按相同规则从当前图枚举 NOOP/BIND/BIRTH/REACTIVATE/RELINK/RETRACT/SPLIT/MERGE 与复合 REPLACE；reference 类型另行构造并靠执行后世界匹配候选。它不读取 future、不使用 learned proposer，也不是 Projective Node Orbit（PNO）或正式规模数据。当前受控 fixture 的 reference 参数仍与候选共用 current-world 排序，所以这里只能叫 label-blind 接口，不能叫独立真值 coverage；完整 20-step 验证也因宿主 `0xC0000005` 中止。

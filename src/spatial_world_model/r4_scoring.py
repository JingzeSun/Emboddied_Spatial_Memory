"""Independent R4 labels, task metrics, selection and family statistics.
...
from .r4_query import STEPS, validate_controls, validate_prediction, validate_times


VERSION = "spatial-history-r4-scoring-v1"
WORLD_IDS = ("LL", "LR", "RL", "RR")
LABEL_KEYS = ("prediction_times_s object_position_m interval_contact task_success "
...
def validate_labels(value):
    ...
    require(all(v is None or type(v) is int and 0 <= v <= 4096
...
def labels_from_trajectory(samples, assessment):
    ...
    result = {"prediction_times_s": [k / 10 for k in range(1, STEPS + 1)],
...
def select_candidates(predictions):
    require(isinstance(predictions, list) and len(predictions) == 4, "four registered candidates required")
    ...
    return [1 / len(tied) if i in tied else 0.0 for i in range(4)]


def score_world(predictions, labels):
    """Four fixed slots; None/malformed rows remain failures in the denominator."""
    require(isinstance(predictions, list) and len(predictions) == 4, "four prediction slots required")
    require(isinstance(labels, list) and len(labels) == 4, "four label slots required")
    ...
    return {"valid": valid, "registered_candidates": 4,
...
def summarize_registered(rows, registry):
    ...
    missing_score = score_world([None] * 4, [None] * 4)

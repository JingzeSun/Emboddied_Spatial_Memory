"""Public causal surface map for the joint c/d engineering audit (D-089).
...
VERSION = "spatial-history-r4-observed-map-v1"
...
PARAMETERS = {"cell_m": .01, "origin_xy_m": [0.,0.], "minimum_ground_axis_extent_m": .3,
              ...
              "dynamic_rule": "same_frame_public_candidates_no_future_association"}
...
def build_map(history, public_sensor_spec, common_shape, map_parameters, *, history_mode="full", history_cut_index=None):
    ...
    for local,(frame,index) in enumerate(zip(frames,indices)):
        result=objects.frame_candidates(frame,public_sensor_spec,common_shape,objects.parameters(),source_index=index)
        geometry=objects._Geometry(frame)
        stats={"local_index":local,"source_index":index,"time_s":frame["time_s"],
               ...
               "unexcluded_components":0,"static_eligible_components":0,"supported_quads":0}
        for component in result["components"]:
            pixels=set(map(tuple,component["support_pixels"]))
            if component["classification"]!="incompatible_extent":
                role="accepted_components" if component["classification"]=="accepted_candidate" else "unexcluded_components"
                stats[role]+=1
    ...
    if indices[-2:]==[119,120]:
        object_state=objects.associate_objects({"schema_version":objects.HISTORY_VERSION,"frames":frames[-2:]},
                                               public_sensor_spec,common_shape,objects.parameters())
    return {"schema_version":VERSION,"status":"nominal_map_ready" if ground else "ground_unresolved",
            ...
            "assumptions":["lowest_large_horizontal_plane_is_support","locally_horizontal_quad_interpolation",
                           "static_large_surfaces","observed_wall_tops_extruded_to_observed_support",

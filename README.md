...
| 目录 | 职责 |
|---|---|
| `src/spatial_world_model/` | 新空间历史合同；与旧科学模块独立 |
...
当前先交付成对数据合同、手工输入输出例子与服务器工程检查，再审真实模拟器生成和重放；阶段命令见 PLAN。长历史与地图加简单动力学是强制对照。暂不开展模型训练、主动探索、开放世界、复杂操作或视频生成。旧 CPMT 代码、科学 no-go、结果与 test 封存状态保留。

...
from r4_examples_v2 import public, prediction
from spatial_world_model.r4_query_v2 import (domain_spec, from_public_query, mean_predictions,
                                        validate_prediction, validate_query)


class R4QueryV2Tests(unittest.TestCase):
    def test_full_recent_and_prefix_clocks_preserve_raw_depth(self):
        for mode, indices, cut in (("full", range(121), None), ("recent", [119, 120], None),
            ...
            self.assertEqual(set(q), {"schema_version", "history", "controls", "goal", "domain_spec"})
    ...
    def test_goal_translation_allowed_but_task_redefinition_rejected(self):
        ...
        for row in src["history"]:
            row["camera_position_m"][0] = 0.1
        from_public_query(src, history_mode="recent")
    ...
    def test_versions_reject_legacy_and_model_features_exclude_envelope(self):
        from spatial_world_model import r4_query as old
        from spatial_world_model.r4_query_v2 import model_features, VERSION, PREDICTION_VERSION
        from r4_examples import public as old_public, prediction as old_prediction
        q = from_public_query(public(), history_mode="recent")
        self.assertEqual(q["schema_version"], VERSION)
        self.assertEqual(set(model_features(q, history_mode="recent")), {"history", "controls", "goal", "domain_spec"})
        with self.assertRaises(ValueError): old.validate_query(q, history_mode="recent")
        with self.assertRaises(ValueError): from_public_query(old_public(), history_mode="recent")
        with self.assertRaises(ValueError): validate_prediction(old_prediction())
        with self.assertRaises(ValueError): old.validate_prediction(prediction())
        self.assertEqual(mean_predictions([prediction()])["schema_version"], PREDICTION_VERSION)
        for value in (None, old.VERSION):
            q["schema_version"] = value
            with self.assertRaises(ValueError): validate_query(q, history_mode="recent")

    def test_rig_rejects_wrong_intrinsics_terminal_pose_and_old_native_pixels(self):
        for key, value in (("intrinsics", [100,100,39.5,39.5]),
                           ("camera_position_m", [0,-0.1,1.4]),
                           ("camera_xyzw", [0,0,0,1]), ("rgb", [0]*12288),
                           ("depth_m", [1.4]*4096)):
            src = public()
            src["history"][0][key] = value
            with self.subTest(key=key), self.assertRaises(ValueError):
                from_public_query(src, history_mode="recent")
        src = public()
        src["history"][0]["camera_xyzw"] = [-1,0,0,0]
        from_public_query(src, history_mode="recent")

    def test_nine_query_registry_rejects_reordering_duplicates_and_changed_history(self):
        from spatial_world_model.r4_query_v2 import validate_candidate_queries
        queries = []
        for i in range(9):
            src = public()
            src["controls"][0]["ee_velocity_mps"][0] = i / 100
            queries.append(from_public_query(src, history_mode="recent"))
        controls = [deepcopy(q["controls"]) for q in queries]
        validate_candidate_queries(queries, registered_controls=controls, history_mode="recent")
        bad = list(queries)
        bad[0], bad[1] = bad[1], bad[0]
        with self.assertRaises(ValueError):
            validate_candidate_queries(bad, registered_controls=controls, history_mode="recent")
        with self.assertRaises(ValueError):
            validate_candidate_queries(queries[:4], registered_controls=controls, history_mode="recent")
        duplicate = deepcopy(controls)
        duplicate[8] = duplicate[0]
        with self.assertRaises(ValueError):
            validate_candidate_queries(queries, registered_controls=duplicate, history_mode="recent")
        queries[8]["history"]["rgb"][0][0] = 1
        with self.assertRaises(ValueError):
            validate_candidate_queries(queries, registered_controls=controls, history_mode="recent")

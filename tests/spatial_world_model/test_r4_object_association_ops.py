...
ops = importlib.import_module("r4_object_association_check")
...
class R4ObjectAssociationOpsTests(unittest.TestCase):
    def test_configuration_matches_scientific_constants_and_census(self):
        from spatial_world_model import r4_object_association as c
        config = ops.configuration()
        proposal = ops.read(ops.ROOT / ops.PROPOSAL)
        self.assertEqual(c.parameters(), {k: proposal[k] for k in ("segmentation", "fit")})
        self.assertEqual(c.common_shape_spec(), {k: proposal["common_shape"][k] for k in c.common_shape_spec()})
        self.assertEqual(config["accepted_proposal_sha256"], ops.sha((ops.ROOT / ops.PROPOSAL).read_bytes()))

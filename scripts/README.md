...
`generate_m1_rollouts.py` 只开放 `train` 和 `validation`，生成真正按前一步 post-graph 继续的 20-decision 程序化世界链。输入是冻结配置、split 和 paired group 数，输出写入 `outputs/m1_rollout/<run_id>/`，包括逐步 online JSONL、整条 audit sequence、配置、摘要和标准 manifest；例如 `--split validation --paired-groups 4` 会生成 4 对 sibling、共 160 个有序决策。白话说，它检查“模型第 4 步改错记忆后，第 5 步是否真的拿错后的记忆继续”，并提供相同当前输入但合法未来不同的 paired pivot，输出可供 A–F runner 做因果 self-rollout；它不是 PNO/视觉数据，不是正式 K=16 数据，也不运行训练或 test。

该入口现在默认使用 `--paired-groups`：每组两条 sequence 共享初始世界、资产、事件计划和 ambiguity pivot 的 online payload，但 pivot 的 audit-only reference 分别指向 primary/contrast；`--sequences` 只保留未配对的接口诊断模式。输入不变，输出额外记录 paired group、sibling 和 pivot；例如 `--paired-groups 4` 产生 8 条 sequence、160 个决策。它不表示整条分叉后的两个世界仍应逐帧 online 相同，也不等于正式 K=16 数据。

`run_m1_af_rollout.py` 使用 `configs/m1_af_smoke.json` 把 A–F 接到 paired continuous rollout。输入是冻结 hard config、明确标为 nonformal 的 smoke config 和 train/validation 数据，输出包括共享 130 维 online tensors、99 维训练期 future targets、A–E 权重、E 的额外 scorer、逐方法 teacher-forced/causal 指标、完整失败和标准 manifest。例如 A 单步选错后，runner 会从 A 自己的 post graph 构造下一个候选。它不访问 test、不自动调阈值，也不是正式 K=16/五 seeds/10,000 bootstrap 实验；当前运行只测接口和本地资源。

`run_m1_trainability.py` 使用 `configs/m1_trainability_ladder.json` 在扩 K=16 前做可学习性审计。输入是相同 train/validation paired 数据、全标签容量曲线和 4→10 group、60→1000 step 的受控点，输出是容量上限差、A–F 指标、candidate/teacher/amortization 误差分解、资源和完整重试记录。例如全标签学生若连 97.5% 的 paired 可观测上限都达不到，就先修表示或优化而不扩候选。它明确 `formal_run=false`、`test_access=false`，不是 checkpoint 选择或正式结论。

`generate_m1_trainability_shard.py` 和 `run_m1_trainability_point.py` 是上述 runner 的隔离 worker：前者每次只生成一个完整 paired group，后者每次只训练/评估一个阶梯点；失败 attempt 原样保留，后续 attempt 不复用半成品。输入是父 run 内冻结配置和显式 group/step，输出带 `complete.json` 的独立 shard/point。它们解决 Windows 宿主偶发进程崩溃时的可恢复性，不改变样本、seed、方法或结果，也不能跳过失败组。

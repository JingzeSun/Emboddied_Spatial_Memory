...
`run_m1_af_scaled.py` 解决“预生成数组、共享 commit 校准、causal 评估和 paired 统计必须属于同一次可追溯运行”的问题：输入是 `generate_m1_parallel.py` 产出的 train/validation npz、validation paired-group 数与 seed，先只用固定哈希 calibration 半区选一组 A–E 共用阈值，再只在 report 半区输出 teacher-forced、20-step active/history/recovery、observable oracle 和 10,000 次 paired-group bootstrap。S1 诊断还输出 target-only 最小候选集合、assembled relation oracle 的事后非法选择率及合法/非法 wrong-template 分解、exact-ambiguity capped 读数，以及 E scorer 在 train/calibration/report 的 masked BCE 与候选 teacher accuracy；合法性只用于审计已选结果，不能反向帮助 E 选择。`--student-steps` 仍对 A–E 的 online student 统一，`--scorer-steps` 只控制 E 的额外 outcome scorer 并在报告中单列，未提供时为兼容旧命令才沿用 student 数值。causal 输出同时保存未受自身漂移影响的 initial-step invalid rate 和全轨迹 rate。每个 (方法, seed) causal 结果单独落盘并带完整逐序列选择，生成/训练/导出各自记录代码与数组 provenance。例如 report group 永远不能让某个阈值胜出。它仍标 `formal_run=false`、不训练 backbone、不触碰 test，也不能把 validation 检查冒充正式 gate。

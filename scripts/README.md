...
`run_m1_af_rollout.py` 使用 `configs/m1_af_smoke.json` 把 A–F 接到 paired continuous rollout。输入是 M1-v2 hard config、明确标为 nonformal 的 smoke config 和 train/validation 数据，输出包括共享且随 K 自动定维的 online tensors、候选作用域关系 targets、A–E 权重、E 的额外 scorer、逐方法 teacher-forced/causal/recovery 指标、完整失败和标准 manifest。例如 A 在歧义点选错位置后，下一次相关可见证据会从 A 自己的错误 post graph 生成同一 K=16，其中可含一个补偿 RELINK。它不访问 test、不把旧错误回填为正确，也不是五 seeds/10,000 bootstrap 的正式 gate。
...
`compare_future_target.py` 解决“说 E 变强了必须是测量而非断言”的问题：其他一切固定，只切换 `future.target_representation`（`hashed_tokens` 对 `world_latent`），输出两种表征下 E 的 teacher 准确率、E/A 的学生准确率与 `A−E` 的变化。生成结果按表征缓存，中断可续。若 E 不因此变强，说明哈希目标不是它的限制、`A_vs_E` 可照原样成立；若变强，A 对 E 的优势必须对着更强的基线重新汇报。

`export_run_report.py` 解决“在租用机器上跑出的数字带不回仓库分析”的问题：输入是一个 run 目录，输出是 `results/<name>.json` 一份 KB 量级的汇总，含 af_report、生成 manifest、逐 seed 的 causal 行、嵌套的 planning-only runtime profiles，以及可查证的 provenance——git commit 与是否 dirty、协议 sha256、dataset version、Python/NumPy/PyTorch 版本、主机名与核数。数组仍留在 `outputs/` 且不进 Git。例如两条架构臂各自产生 `runtime_profile.json` 时，exporter 会保留相对路径并装入同一结果文件；它不把成本投影变成准确率或正式实验结果。工作区 dirty 或 causal 未跑满时会显式警告，避免把不可比或缺主指标的结果当成结论。

`run_m1_af_scaled.py` 解决“预生成数组、共享 commit 校准、causal 评估和 paired 统计必须属于同一次可追溯运行”的问题：输入是 `generate_m1_parallel.py` 产出的 train/validation npz、validation paired-group 数、seed 与架构臂，先只用固定哈希 calibration 半区选一组 A–E 共用阈值，再只在 report 半区输出 teacher-forced、20-step active/history/recovery、observable oracle 和 10,000 次 paired-group bootstrap。`--architecture cross_candidate_set_transformer_v1` 让同行 K=16 候选通过无位置编码的自注意力互相比较，输出仍是一候选一分数；例如两个 RELINK 只有相对另一候选更匹配时，注意力提供候选间信息通路。它不等于读取候选执行后世界，也不等于用更大模型替代 CTL：同一架构臂内 A–F 共用学生结构，Set Transformer 是预登记主臂，`shared_candidate_mlp_v1` 是同数据次臂。描述性 report 还输出 target-only 最小候选集合、assembled relation oracle 的事后非法选择率及合法/非法 wrong-template 分解、exact-ambiguity capped 读数，以及 E scorer 在 train/calibration/report 的 masked BCE 与候选 teacher accuracy；这些 report 字段不能用于 S1/S2 选择，合法性也不能反向帮助 E。`--student-steps` 仍对 A–E 的 online student 统一，`--scorer-steps` 只控制 E 的额外 outcome scorer并在报告中单列。causal 输出同时保存未受自身漂移影响的 initial-step invalid rate 和全轨迹 rate。每个 (方法, seed) causal 结果单独落盘并带完整逐序列选择。它仍标 `formal_run=false`、不训练 backbone、不触碰 test，也不能把 validation 检查冒充正式 gate。

`run_m1_scorer_diagnostics.py` 是 S1/S2 专用的 train/inner-dev 入口，解决“诊断 target 或调 E scorer 时不应反复查看 validation report”的问题。输入只有 train arrays；它按 canonical paired-group ID 的 SHA-256 固定留出约 1/5 完整 group，在其余 train group 上拟合 scorer，在 inner-dev 上报告 target-only、assembled oracle、masked BCE、teacher accuracy 和非法选择分解。例如 10 个 train groups 中同一 sibling 的 20 个在线步骤和 recovery row 要么全部训练、要么全部留出。输出明确写 `validation_arrays_read=false`、不训练 online student、不校准 gate、不跑 causal，也不等于新增正式数据 split；正式 config 在 S4 重新冻结时才登记最终选择规则。

`generate_m1_parallel.py` 解决“正式规模的数据生成是 CPU 上的墙钟瓶颈”的问题：输入是冻结 hard config、split 与 `--paired-groups`，输出是与串行逐字节相同的合并 arrays 加一份 manifest。该参数是覆盖 C00–C11 的**总混合 paired-group 数**，例如 train 传 1000 就生成总计 1000 groups，不再乘 12；runner 同时把逐候选六项能量、raw now/future、family 编号、causal/learning group support、行为指纹、C10/C11 机制门、同分母 current-now 审计和 teacher/reference 健康门写入 arrays/manifest。行为指纹的输入是真实 reference、观测有效性和 legal collateral 对照，输出是 12 个 family 的机制摘要与重复检测；例如 C11 必须每行存在合法但误改无关开放事实的候选，它不等于检查 family 标签。每个 paired group 只依赖自己的种子，因此按组分发到进程、各自写 shard，父进程按组序合并并恢复串行的 group 编号；`--verify` 会额外跑一遍串行并要求数组完全相同。它只是 runner，不训练、不评估、不触碰封存的 test；train 健康门失败时产物保留供审计但返回非零，明确禁止进入训练。

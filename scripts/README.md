...
`generate_m1_rollouts.py` 只开放 `train` 和 `validation`，生成真正按前一步 post-graph 继续的 20-decision 程序化世界链。输入是冻结配置、split 和 paired group 数，输出写入 `outputs/m1_rollout/<run_id>/`，包括逐步 online JSONL、整条 audit sequence、配置、摘要和标准 manifest；例如 `--split validation --paired-groups 4` 会生成 4 对 sibling、共 160 个有序决策。白话说，它检查“模型第 4 步改错记忆后，第 5 步是否真的拿错后的记忆继续”，并为每步使用固定、label-blind 的 K=16 候选；它不是 PNO/视觉数据，也不运行训练或 test。K=16 的单步执行已通过，完整链路验证因宿主 Python access violation 中止，当前仍标未验证完成。
...
`audit_m1_candidates.py` 是 K=16 的训练前 coverage 专用入口。输入是当前 world、train/validation split 和 paired-group 数，输出是逐决策 reference canonical match、总体与逐 family coverage、非法 reference、去重数和 gate 状态；它不构造 future trace、不训练 A–F，也没有 test 选项。例如隐藏 reference 为 RETRACT 时，生成器仍先枚举全部 16 个候选，再把另行执行的 RETRACT 后世界与候选后世界匹配；匹配不到就明确记为 candidate miss，不能算成 CTL/scorer 错误。当前受控 fixture 只把隐藏事务类型与生成器分开，reference 的 node/edge 参数仍共用 current-world 排序，因此 summary 会写 `reference_arguments_independent=false`，不能把其 coverage 数值当正式 gate。该入口已实现，但因宿主不稳定尚未运行完整审计。

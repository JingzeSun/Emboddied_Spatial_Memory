...
`generate_m1_parallel.py` 解决“正式规模的数据生成是 CPU 上的墙钟瓶颈”的问题：输入是冻结 hard config、split 与 `--groups-per-family`，输出是与串行逐字节相同的合并 arrays 加一份 manifest。该参数是每个 C00–C11 family 的 paired-group 数，例如 train 传 1000 会生成 12,000 groups，不是总共 1000；runner 同时把逐候选六项能量、raw now/future、family 编号和 teacher/reference 健康门写入 arrays/manifest。每个 paired group 只依赖自己的种子，因此按组分发到进程、各自写 shard，父进程按组序合并并恢复串行的 group 编号；`--verify` 会额外跑一遍串行并要求数组完全相同。它只是 runner，不训练、不评估、不触碰封存的 test；train 健康门失败时产物保留供审计但返回非零，明确禁止进入训练。

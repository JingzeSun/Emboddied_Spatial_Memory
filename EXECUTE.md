...
## LOG-133：D-116确认E0像素相位退化，不支持身体遮挡归因（2026-09-13）

- `6d2449b`登记并实现固定快照像素相位归因，49项原E0、独立评估和新增边界面人工例在服务器全部通过。正式运行读取21个已登记源文件，先封存公共深度/E0输出，再用原XML和分割图独立审计；4世界×2门×33相位共264次重渲染，0次`mj_step`、0训练、0确认读取，原源文件前后摘要相同。运行与审计耗时35.8022秒，输出268个封存文件。
- 零相位逐值复现原8个指定观察结果，唯一失败仍为LL/LR近门。两者右门边均投影到`u=58.003943790670576`；第58列在39–43行命中`gate_0_right`竖直内侧面，实际深度`1.1002326011657715 m`，比E0墙顶区间远端加0.2 mm的要求少`0.06742267608639274 mm`。共10个失败像素全部是门墙，物块/推头阻挡计数为0，因此`body_occlusion_supported=false`。
- 墙顶平面像素间距约10.5563 mm。零点两侧最小登记横移仅±1/32像素，即±0.329883 mm，就分别把右边界投影改为`u=58.03519379067058`和`u=57.97269379067058`，两侧E0均从0候选/5个`gap_not_farther`变为1候选/0个该拒绝；LL/LR在其余全部32个非零相位也通过。原本通过的另外6个世界–门组合在33个相位全部通过。正式回执因此给出`pixel_phase_causal_attribution_supported=true`。
- 白话：门、机器人、物块、相机前后位置和E0都没换，只让门边在图像上左右挪一点；恰好擦中第58列时该像素既没进入墙顶连续run，又没有比墙顶低到E0要求的程度，所以整道门失败。离开这个亚像素位置立即通过。这是渲染栅格与E0保守间隙规则的确定性边界退化，不是身体挡住、物理/replay失败或三模型失败。
- 报告[spatial_history_r4_e0_pixel_phase_v1.json](results/spatial_history_r4_e0_pixel_phase_v1.json)由`7f447e1`回传，894,977 bytes，SHA-256=`96b6864d4e99e7861b0b9cda5aacb471aecb4df0175a66c30cae054d491c9994`。它只完成`r4-36`原因归属，不证明任意横移轨迹可覆盖64家族，也不选择数据准入语义。SH-05继续硬停止，等待用户在人为语义上决定：E0是否仍是数据准入门、是否把缺边界保留为M/E0的基线失败，或是否建立原则性的边界像素E0新版本。

## LOG-134：D-117冻结E0-v2边界兼容语义并通过独立人工检查（2026-09-13）

- 用户批准边界B像素只扩大墙区间、不认证自由；有效但过窄的区间继续交悲观M并正常计错；无有序边界、无严格更远中央F或公共冲突继续在动力学前硬停止。`e5bff8a`据此新增独立`public-openings-v2`模块、审查配置和人工正反例，E0-v1源码/配置/结果均未改，M尚未接入v2。
- E0-v2保持原0.2 mm严格更远分离量并将所有数值和声明锁死。合法行只能是`B* F+ B*`，前缘只能是墙顶后接`B* F`；B跨入中央、全B、较近遮挡或无效深度均不形成候选。边界支持保存墙顶端点、最近严格F、夹在其间的全部B及扩大后的完整像素足迹区间，不输出自由格。
- 服务器首次调用因`PYTHONPATH=.`在收集前找不到`src/spatial_world_model`，0项实际测试；改为`PYTHONPATH=src`后42项中39项通过，唯一失败函数的3个subtest均因夹具构造篡改配置后误调用仍绑定原配置的helper。`fe71e45`只把该行改为将篡改配置实际传入入口，未改生产代码、配置或语义；随后E0-v1 28项、E0-v2 10项、D-116像素相位4项共42/42通过，用时0.147秒。
- 本批0模拟、0数据生成、0训练、0确认读取，也未在r4-36真实快照上运行E0-v2。通过只证明独立人工规则与E0-v1回归成立；下一步仍须用户审查`e5bff8a..fe71e45`，审过后才建立新目录运行既有33相位公开扫描，结果封存后再作私有归因评价。

## LOG-135：E0-v2固定33相位检查264/264通过且原262项逐值保持（2026-09-13）

- `278ae70`固定D-118阶段后，服务器首轮45/45检查通过；首目录复用并封存264项公开输出后，在创建第一个私有MuJoCo renderer前因shell缺EGL环境停止。`failure.json`保存OpenGL context错误和14.6199秒，0私有分割、0物理步；该目录未覆盖。`7a9750d`只显式固定`MUJOCO_GL/PYOPENGL_PLATFORM=egl`并换独立目录，未改E0-v2、输入或通过门；新绑定45/45通过，用时0.167秒。
- 正式运行逐摘要复用D-116的264张封存RGBD，0新公开渲染；全部v2预测封存后才从同一快照私有重渲染264次并逐值核对观察，再读取XML目标和分割geom。运行26.6699秒、0`mj_step`、0数据生成、0训练、0确认读取，源D-116目录与报告绑定保持。
- E0-v1事实仍为262通过/2失败；E0-v2为264/264通过。原262项均未消费候选B，去掉v2新增溯源字段后候选坐标区间、平面和逐支持像素与v1完全相同。原LL/LR零相位两项各用5个`x_right` B恢复；10/10均命中`gate_0_right`，物块/推头命中0。两项右边界区间宽`0.010560493144025074 m`、真值包含且中点误差约`0.0000003363 m`；左边/前缘宽约21.1491/21.1222 mm，全部低于原25 mm门。
- 报告[spatial_history_r4_e0_boundary_phase_v2.json](results/spatial_history_r4_e0_boundary_phase_v2.json)由`5faffbb`回传，774,439 bytes，SHA-256=`4040442da758a45342ee451b5d6c14b75aa415e75f449368c4a1641798092b80`。这支持E0-v2消除已定位的单点像素相位退化且不改变同一扫描中的原成功几何；它尚未验证其他家族/视角，不授权把v2接入M、恢复剩余生成、重建数据或训练。

## LOG-136：E0-v2在50个封存开发家族800项准入评价中全部通过（2026-09-13）

- `24e0ab2`按D-120固定50家族：4个原工程家族、44个生成阶段已验证家族，以及仅作E0证据的`r4-22/r4-36`；未封存的`r4-23/r4-58`排除并保留。服务器41/41检查通过，50份`public_manifest`与当前四历史文件逐摘要相同。正式运行先只读公开历史并封存800份v1/v2输出，之后才读取原预测、family result、XML、快照和分割；0新公开渲染、0物理步、0训练、0确认读取。
- E0-v1的800项与原封存输出逐值相同，仍为796通过/4失败；失败身份固定为`r4-36-LL-full/view_a`与`r4-36-LR-full/view_a`。E0-v2为800/800通过，原796项全部保持通过，没有B的原成功项候选几何逐值不变，原4项失败均以非空B支持恢复。最大匹配坐标区间宽`0.0211759406301551 m`，低于原25 mm上限。

...
**2026-09-24 最新暂停点：裁决 57 已冻结召回三值（k=5／k′=3／3 m），裁决 56 (a) 的只读估算已完成（LOG-244 续）；S2 仍等待两项裁决：匹配口径本身（待裁 56 续）与出生半径（待裁 58）。** 估算显示把 IoU 门拉到接近 0 的是 wall／room／door／window 四类建筑结构（占 65% 的行）；排除它们后，同帧并集框中位 0.368 过门，单视角 0.165、跨帧累积 0.129 不过。

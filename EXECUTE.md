...
| 事项 | 已知事实 |
|---|---|
| VSMT首篇/VM-01～03 | 日常入口和论文定位已收敛为VSMT；公私合同、TAF/ELU/WFR/LOW clean-room适配器、八原子加REPLACE公开候选与封存后teacher已形成代码候选；新增拒绝旧语义LATENT和禁止导入旧M1 query/feature的反作弊检查，共36项定向测试待服务器运行。阈值/数据/训练均未冻结或执行；LOG-133，D-125 |

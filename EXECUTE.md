...
| 事项 | 已知事实 |
|---|---|
| VSMT首篇/VM-01～04 | D-140～D-147工程基线已由服务器179项合同与61关系边/6候选/9桶/0截断/341可见截锥smoke认证，报告`b5918086…c68eff`绑定`33aec1c`。D-148按D-144数值完成完整2-house固定入口：只读inventory、独立哈希选房/36-slot计划、双family模拟器worker、公私文件隔离、trusted L1匿名化、三profile×16/32/64 public replay与seal、post-seal strict canonical/private recall、2/5 cm RETRACT合法性、资源停止、verify/export；本地executor 42/L1 31/VSMT 131共204项通过，当前停在唯一一次总审。统计独立单位仍为family，L2/SAM后置，逐版本模板归属留到五方法比较前；当前0 source inventory、0 house选择、0 VM-04数据生成、0private评价、0训练/validation效果/confirmation。LOG-133–148，D-125–148 |
...
- 固定入口`ops/vsmt/vm04_two_house_audit.py`完整覆盖`contracts → inventory → select → capacity → generate → materialize → public-seal → private-eval → verify → export`。inventory只读本地author-train JSON/JSONL/gzip和显式license，不下载；select必须是下一次命令，按冻结manifest/seed/house ID选两个house并生成一次性私有salt。生成由两个Python 3.9 worker各守一个family和18个固定slot，原始RGB/depth/camera与instance ID/mask/干预分目录保存；失败不换house。
...
- 资源策略在启动前检查1800秒、4 GiB、8 GiB磁盘保留线和双worker；运行中每family超过2 GiB时保留当前失败、把余下固定slot逐个记`not_started`并停止新分派。verify要求36个slot都有complete/failed/not_started终态、两个worker都有exit、公开seal先于私有评价、全部摘要绑定及合并摘要不依赖worker完成顺序；export用O_EXCL写新报告，不覆盖既有结果。

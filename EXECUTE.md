...
| SH-04-R3/E0公开几何 | D-078在原恢复/评估器上交付默认4路、可选1–16路入口；97项服务器检查与16项真实查询均待运行，尚无几何通过、加速或模型结果。LOG-110 |
...
## LOG-110：E0公开开口恢复与独立评估实现交付（服务器待运行）
...
- 实际59项测试的通过/失败/退出、16个真实查询（4世界×full/A/B/recent）、500个提取器输入帧、各边包络误差、候选数量/冲突、近期一致性、来源不变性及资源测量均pending。正式run和首次成功export合计≤1800 s、阶段及报告合计≤64 MiB、存活进程树合计RSS≤512 MiB为执行门，不是已达成的实测值；来源核验和终检包含在run中。临时回执/报告先写后验再发布，失败与硬中断保留；新阶段目录和报告路径见PLAN，原R2/R3不重跑或覆盖。
...
- D-078并发交付补充：用户确认0b1640f串行版尚未启动，并要求采用旧M1/S5的worker方式。新入口支持默认4、可选1–16路公共查询，实际恢复/评估源码及其45项科学检查字节未变；16查询和500个提取器输入帧不变。新的运维测试清单为21项入口、20项调度、11项资源，总97项，全部服务器待运行。测试包含小型标准库1路/4路逐字节一致与两进程竞争同一磁盘额度的检查，不能把这些手工任务当作实际几何一致性或加速结果。
...
- 并发版本配置及目录独立，原串行配置、提案、0b1640f版本与全部R2/R3产物保持原样；旧E0目录如已存在则拒绝自动另跑。内存门明确由原整体512 MiB调整为`(workers+1)×512 MiB`，另有512 MiB启动余量，4/8/16路整体门为2.5/4.5/8.5 GiB；1800 s及64 MiB预算不变。CPU/RAM实际配额、并行耗时/峰值、测试退出与几何结果仍pending；本地仅静态检查，没有执行新旧模块、测试、模拟或训练。

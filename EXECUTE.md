...
**2026-09-24 最新暂停点：S1-03 cache 在 `5f9aa71` 的 39 条上重建完成（39/39），S1-04 前端诊断跑完（39/39），IoU 继续门被触发，S2 等待待裁 56（匹配口径）与 57（召回值冻结）（LOG-244）。** fragment-真值 IoU 中位 0.0004＜0.3：可拾取 0.45、容器 0.14，非可拾取非容器占 76% 的行且中位≈0；召回曲线在 k=5、k′=3、半径 3 m 时漏召回 2.0%；ReID 选择规则算出 ViT-B/14 投影（增益 0.093），由 S1-05 执行。

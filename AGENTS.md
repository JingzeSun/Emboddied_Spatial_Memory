...
白话：这些规则解决“一个长命令中间失败后窗口消失、路径猜错、昂贵步骤重复跑”的问题。输入是当前已确认的仓库路径和上一功能块产物，输出是一小段可单独验证和续跑的命令。例如 40-group arrays 已有 digest 时，下一块只训练 scorer，不能再次从 unittest 开始。它不等于把每一行命令都拆成一轮对话，也不改变实验方法或统计协议。

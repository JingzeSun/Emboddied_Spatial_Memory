...
- 用户于 2026-09-19 批准 D-224 裁决 A～D：首篇精简为 **VSMT-lean（实体生命周期版本化事务）**，模型为冻结 SAM 2.1 + DINOv2 前端、三个约 4 万参数的 MLP 代价头和每帧一次矩形匈牙利分配；数据为 ProcTHOR 多 house 覆盖式重访加不可观测窗口干预；指标对齐 Dyn-THOR。
...
- S0-01 实体记忆合同已审通过，S0-02 干预数据合同已实现待审；数据生成、训练、服务器运行全部未授权，合同内数值全为 null。D-059 的单职责科学提交、用户代码审查和服务器规则继续有效。

## 上一方向（D-122，已由 D-224 取代，仅用于旧代码和旧运行审计）
...
- 服务器上的数据生成、超参数搜索/调参、训练、验证、测试、检验和审计，只要存在两个或以上可独立执行的工作单元，就必须使用多个 worker，不得默认串行，也不得未经资源核验固定为少量 worker。运行前根据可用 CPU 核心、GPU 数量与显存、内存、I/O、模拟器并发安全性和单 worker 实测占用，选择当时的最大安全 worker 数；GPU 单卡训练至少并行化数据加载或其他可安全分片环节，不得为满足形式而启动会争抢显存、降低吞吐或改变数值语义的多个训练进程。入口必须记录 requested/actual worker 数、资源依据、任务分片、退出状态和确定性合并顺序。只有任务本质串行或上游组件经实测不支持并发时才允许单 worker，并须在启动前明确说明原因、证据和预计时间；不得运行很久后才发现入口是串行。已经运行的任务不因本条自动中断，是否停跑重启仍由用户明确决定。
- 正式服务器生成、调参、训练、验证、测试、检验和审计不得因为预设墙钟时长到达而强制失败；任务需要运行多久就允许运行多久，预计超过30分钟时按既有规则转后台并持续保存进度、退出和续跑证据。仍必须保留内存、显存、磁盘剩余量及异常失控进程的安全保护，避免机器崩溃、数据盘写满或破坏其他产物；这些安全线不能伪装成较短的计算时间预算。若论文实验需要统计计算预算，只记录实际用量或另行冻结用于公平比较的训练步数/样本数，不用墙钟超时截断合法任务。
...
- 凡下一步需要用户认可、裁决或冻结，最终回复必须在对话正文直接列出：待决事项、推荐口径、其他可选口径、各自对结果/资源/主张的影响，以及用户可直接复制回复的简短批准句；不得只给文件链接或要求用户自行搜索文档。文件链接只作证据。若本轮无需用户决定，明确写“本轮无需裁决”。

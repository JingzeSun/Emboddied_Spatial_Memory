...
| 节点 P / R / F1（质心次级列，裁决 70） | 同一批预测实体、同一批真值物体、同一个评价器自带的最大权匹配，只把“3D IoU ≥ 0.3”换成“实体质心到真值质心距离 ≤ δ_moved（已冻结 0.5 m）”，权重 1／(1＋距离)；与主列并列报告，不选配置、不进主门。白话：它把“记错地方”和“地方对、框太薄”分开——实体框来自单帧深度表面壳，对整体真值盒的 IoU 天然偏低（LOG-255：TAF 下 83～87％ 的真值物体附近有身份正确的实体，35％ 却因 IoU＜0.3 落选）。它不是主指标，也不替代 IoU 0.3 的 Dyn-THOR 对齐口径 | 自家（诊断列） |

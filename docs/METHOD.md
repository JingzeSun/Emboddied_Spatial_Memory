...
首轮拟采用固定物理参数、一个平面运动物块、一个笛卡尔推杆和固定观察路径。Cartesian pusher（笛卡尔推杆）是可按世界坐标速度指令运动的简化机器人末端；输入每段速度和持续时间，输出机器人及物块响应，例如向左前方推。这不是复杂机械臂抓取，也不能把物块实际位移当作速度控制。具体模拟器、接触/控制实现和场景数值仍待下一职责批次审查；首轮合同的示例不代表物理实现。

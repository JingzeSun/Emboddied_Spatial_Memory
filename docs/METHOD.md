...
| 指标 | 定义 | 对齐 |
|---|---|---|
| 节点 precision / recall / F1（主列与选参指标，裁决 72 (B) 把裁决 70 改取 (b)） | 每帧按评价器自带的最大权匹配，把仍在记忆里（`active` 或 `dormant`，裁决 L）的实体配给本帧在场且至少可观察过一次的真值物体（裁决 Q）；一对能配的条件是实体质心到真值质心距离 ≤ δ_moved（已冻结 0.5 m），权重 1／(1＋距离) 让更近的对先配；墙、房间（地面）、门、窗与房间天花板不进真值范围（裁决 56 续、69：一次只看到一段墙而真值框是整面墙，它们也不是可干预的物体；论文里的“节点”只指物体，不含房屋结构）；解析到在场但范围外物体的实体退出精确率分母、单独计数（复审修订，LOG-225）。白话：它回答“实体记在了对的地方吗”。例如一把椅子的实体框只是它朝向相机的那层表面，质心离真值质心 0.2 m，算配上；另一个实体停在 1.3 m 外的旧位置，算不上。它不等于框重叠 | Dyn-THOR 同一匹配机制、不同重叠判定 |
| 节点 P / R / F1（IoU 次级列，`node_prf1_iou`） | 同一批预测实体、同一批真值物体、同一个匹配器，只把重叠判定换成 3D AABB IoU ≥ 0.3（裁决 C 原主列），权重为 IoU；与主列并列报告，不选配置、不进主门。白话：它回答“框重叠够吗”，也就是 Dyn-THOR 原口径的数值。实体框来自单帧深度表面壳、真值是整体盒，这一列天然偏低：完美分割＋按私有身份完美合并时上界也只有 0.64（LOG-257 续），所以它用于与 Dyn-THOR 对照，不用于判断方法 | Dyn-THOR 原口径 |

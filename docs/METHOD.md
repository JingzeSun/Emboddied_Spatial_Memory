...
| `HeuristicLabel` | 用 ELU-P 在公开数据上的决定当标签训练同一网络（D-224-SW 裁决 T；TAF 从不撤回，存在头会没有 gone 样本可学） | 私有实例真值相对公开启发式标签的贡献 |
...
| 指标 | 定义 | 对齐 |
|---|---|---|
| 节点 precision / recall / F1 | 每帧按最大权匹配与 3D IoU 0.3 把仍在记忆里（`active` 或 `dormant`，裁决 L）的实体框匹配到本帧在场且至少可观察过一次的真值物体框（裁决 Q） | Dyn-THOR |

...
**为什么主表换成实例分割（裁决 72，依据 LOG-257 续）。** 同 4 条开发 episode、同一 runner／teacher／审计，只把色块来源从 SAM2 换成模拟器实例分割：TAF（θ_a 0.65 无门）节点 F1（IoU 0.3）0.224 → 0.399，每真值实体 3.19 → 2.42，"从未出过色块"的真值 4.0％ → 0；按私有身份完美合并的上界 IoU 口径 0.45 → 0.64、质心口径 0.76 → 0.86。结论是 SAM2 的过分割与漏检占低 F1 的一半，是与记忆方法无关的混杂因素；参照 DINO-WM"冻结预训练特征＋很小的学习模块＋刻意简单的环境"的做法，主表用无混杂的分割，把方法差异留在陈旧实体、假撤回与身份连续上。**它不等于把身份告诉方法**：cache 里没有任何实例标签，同一物体在两帧里的两个色块是否同一实体仍由方法从外观与位置推断；它也不修正单帧深度表面壳与整体真值盒的几何差（完美分割＋完美合并时 IoU 0.3 上界仍只有 0.64）。**限制**：主表结论只在"分割已知"的设定下成立；SAM2 前端在同一批开发 house 上报一次作鲁棒性附录，论文须并列说明。ReID 投影头（裁决 47）是在 SAM2 色块上训练的，默认原样复用冻结权重、在新 cache 上只复核召回曲线；是否重训另提裁决。

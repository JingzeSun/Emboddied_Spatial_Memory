...
以下推荐值只是项目建议，`pending` 不等于已经接受。D-024 后，当前 posterior pilot 的直接冻结集是 HC-004/005/013/015/016/017/018/019；HC-001/002/003/011 只在其语义进入 canonical upstream/dependency 时冻结；HC-006–010/012/014 是未来集成项，不得阻塞当前设计，也不得因延期而自动 accepted。旧板块路径仅保留历史追溯，当前映射以 `experiments/bounded_revision_validation/CRITERIA.md` 为准。
...
| HC-018 | 后验方案的第一批可执行数据先做手写 smoke，还是直接从 AI2-THOR 开始？ | 先做 12 个 hand-authored oracle cases，evaluator/deterministic gate 通过后再生成 symbolic 与 AI2-THOR | 决定最早失败是判尺问题还是数据/感知问题；不改变 D-024 的文档主线 | [EXECUTE P0](../EXECUTE.md)、[pilot](03_pilot_protocol.md)、[详细工作表](human_confirmation/HC-018.md) | fixture contracts、phase gate、run manifest decision_ids | pending / — |
| HC-019 | 下游任务错误是否合成单一 cost，若合成权重是什么？ | 主表分报 wrong dispatch、missed invalidation、collateral recompute；暂不合成，行业权重只作次级敏感性分析 | 权重包含应用价值判断，模型或作者不能冒充医院/仓库风险偏好 | [CRITERIA C14](../experiments/bounded_revision_validation/CRITERIA.md)、[详细工作表](human_confirmation/HC-019.md) | evaluation config、task metrics、run manifest decision_ids | pending / — |
...
| [HC-017 baseline fairness](human_confirmation/HC-017.md) | [HC-018 first executable data](human_confirmation/HC-018.md) | [HC-019 task cost](human_confirmation/HC-019.md) |

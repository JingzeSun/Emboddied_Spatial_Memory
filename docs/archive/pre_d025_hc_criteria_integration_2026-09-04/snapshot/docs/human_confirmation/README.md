...
本目录展开 HC-001～HC-019 的场景、输入输出、允许答案和评价标准。D-024 后当前直接冻结集为 HC-004/005/013/015–019；其他条目保留作上游语义或未来集成，不阻塞当前文档设计。
...
posterior-first 的推荐阅读顺序是：先决定 [HC-018](HC-018.md) 的 hand-smoke / AI2-THOR-first；再处理 HC-013/015/016/017（评价、时间、提交门、基线）、HC-004/005（负证据与等价事务）和 [HC-019](HC-019.md)（任务代价）。这只是阅读顺序，不在本文件复制状态或最终答案。
...
文件名当前为 `HC-001.md` 至 `HC-019.md`。新增人工问题必须先在 `DECISIONS.md` 登记新的 HC，再创建对应工作表。

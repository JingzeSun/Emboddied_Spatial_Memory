...
> 方向版本：D-017 + D-021 + D-022 accepted；HC-014 pending
> 更新日期：2026-08-29
...
1. 旧 belief 与动作是否预测下一结构、预期观测和未见空间候选？
2. 当前帧的 patch/region/object/plane 怎样绑定到持久 world node？
3. 新观测到来时，旧状态如何被投影或检索？
4. 差异是像素/feature residual，还是 entity/relation/visibility 级 innovation？
5. 更新单位是 frame、token、voxel、object slot 还是 graph substructure？
6. 候选、确认事实、真实变化和低置信歧义是否分开？
7. 方法是否显式决定 affected scope；关系影响是否传播，在哪里停止？
8. 是否保留历史版本/evidence，并同时评测必要更新与无关保持？
9. posterior 是否用于主动取证或任务行动？
10. 每帧是否隐式重算完整历史？
...
|---|---|---|---|
| 结构区域与几何布局 | LayoutNet、PlaneRCNN、PlanarRecon | VP/layout、单帧 plane masks、跨 fragment plane tracking/fusion | 临时 region 怎样在歧义和动态变化中受控写入版本化 world belief |
...
> 在固定相同 observation、prediction 和 association 输入时，新证据怎样通过 evidence path、typed operation、affected/control/stop 和 version/provenance，有限地改变世界事实的 semantic status、topological relation 与 valid time？

其中“区域切分”“跨视角 plane/feature fusion”“在线图构建”“动态对象记忆”“动作预测”“Top-K 读取”都已有先例，只承担前置或下游角色，不能并列作为 novelty。region-to-world bridge 的具体文献边界见 [结构区域到持久世界节点](structural_region_world_binding_2026.md)。

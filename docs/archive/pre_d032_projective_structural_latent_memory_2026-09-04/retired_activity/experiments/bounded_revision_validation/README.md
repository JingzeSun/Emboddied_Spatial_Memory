...
`CRITERIA.md` 保留 C1–C15 的唯一公式字典；本包每个设计文件又把相关 criterion 作为表格纵列嵌入具体对象：输出变量、模型模块、loss、数据轨、S00–S11、baseline、protocol phase、falsifier、task 和 publication gate。这样可以在当前实验行直接看到“是什么意思、怎么算、数字例子和关联 HC”，同时避免产生第二套公式。

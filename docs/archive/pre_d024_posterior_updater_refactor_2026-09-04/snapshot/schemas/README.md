...
本轮 posterior-first 重构**没有修改 v1.1 schema/config**。planned v1.2 gap 包括：独立 evidence-event record、`observed_at/received_at`、`source_id`、`evidence_group_id`、polarity/coverage、direct targets、supporting evidence 与 learned gate probabilities。只有 HC-015/016/018 及相关语义被接受后，才可把这些字段写入正式 schema；当前只存在于实验模板草案。

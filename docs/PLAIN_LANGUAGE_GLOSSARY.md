...
机器人维护的不是“见过哪些图片”，而是一个带版本的内部世界。每次得到新的观测 latent，它提出几种竞争性的世界解释，分别在同一个旧世界的副本上真实执行，再用后续观测判断哪种修改最合理。训练期从这种比较中学习；部署时不看未来。“新”指观测到达的时间，不是预先告诉模型出现了新对象。
...
这里的 observation latent（观测表征）记为 \(z_{t,i}\)，指当前时刻区域 i 的观测编码；旧 canonical world latent（世界表征）记为 \(m_j\)，指此前已存的世界节点表征。它们解决“同一对象的多次观测如何关联和修订记忆”的问题：输入是当前观测、截至当前的位姿/可见性及旧记忆，输出是保留、关联或修订旧记忆的事务。例如椅子的侧面观测虽然是新的 \(z_{t,i}\)，仍可能支持旧椅子 \(m_j\)；它不等于出现新实体，也不意味着每个新向量都要覆盖旧向量。

“旧 latent 受到影响”须区分两层：

| 层次 | 具体含义 | 当前证据边界 |
|---|---|---|
| 旧记忆的结构及引用修订 | 改身份关联、位置关系、证据归属、生命周期或 latent 引用集合 | M1 事务与连续评测覆盖这一层；合法执行仍可能选错，图/证据正确性须另评 |
| 旧世界 latent 的数值修订 | 改写 canonical 向量的分量，改变它对当前及其他视角观测的解释 | planned；现有 M1 没有学习这种向量更新，也未验证它的稳定性/污染 |

例如 MERGE 可以把两份档案的 `latent_refs` 汇入一个 canonical 版本，但合并引用列表不等于学会融合两条向量。新的观测使后续图状态或固定解析描述发生变化，也不自动证明旧向量被优化过。目标不是一概让旧记忆不变，而是让有证据支持的修订发生、应保留的内容不被误改；改变的幅度本身不能判断好坏。

当前 M1 的 `anonymous_signature` 模拟观测外观 latent，`semantic_latents`/`appearance_of` 提供固定解析外观代理，`latent_refs` 是世界记录中的引用。另一路 `proposal_observation` 检索提示不能仅因也是向量就视为该观测 latent：其 node/edge/place/merge queries 目前存在参考程序目标 ID 派生路径。合法的当前观测输入与这条额外信息通道要分别审查。具体代码映射、影响层次及尚未实现事项见 [M1 实验合同](../experiments/counterfactual_transaction_learning/HARD_CONDITION_EXPERIMENT.md#新观测-latent-与旧记忆的影响边界)。

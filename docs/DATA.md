...
D-177正例条件已实现为独立[私有评价硬门](../ops/vsmt/vm04_relink_positive_gate.py)供代码审查：先核`old-observation-packet.json/prior-memory.json/post-observation-packet.json`及`relink-proof.seal.success.json`摘要，再打开`private/outcome.json`；只有私有前后instance ID相同、prior有公开旧`located_at`证据、post有公开新`located_at`证据、P1/P2公开地点不同、不强制机器人动作已核且真实关系稳定变化六项全真，才写`physical_relink_positive=true`。输出只给六项布尔、失败原因和公私文件摘要，不导出ID；不同实例或任一公开关系缺证据输出false且不产生QUARANTINE。当前仅人工公私文件测试5/5，未接生成stage、未签任何真实正例。白话：它解决“文档说必须同一物体和两端公开证据，但未来评价器可能漏查一项”的问题；输入封存公开证据和事后私有后态，输出正例布尔与缺项，例如同一凳子但P2公开关系缺失就拒绝。它不等于公开模型读取私有ID、候选/teacher评价、在线暂存门或真实数据已通过。

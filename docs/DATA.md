...
| `branches[].future` | 每段控制结束时的 `time_s / object_position_m / contact`；contact拟表示本间隔物块与非机器人障碍发生接触 | 只作监督/评估，正式接触阈值仍待物理合同固定 |
...
下一批数据源为待选的可重放刚体模拟器；需保存原始场景、控制器设置/饱和限制、物理步长、摩擦/质量、真实快照、渲染设置、原始传感器与未来轨迹，并核验字节摘要。所有候选从各自世界同一个实际快照恢复后真实执行，禁止按结果换样本。近期像素、深度和本体状态相同之外，还需检查早期证据确实揭示有关结构及接触期间目标确实不可见；当前验证器没有这些能力。

...
[v3目标边界提案](../configs/vsmt/vm04_target_boundary_proposal_v3.json)是新版本、关闭式合同，字段白话含义如下：`pose_source/private_asset_membership_may_affect_pose`规定仍用v2固定相机位姿且私有资产不得挑镜头；`physical_intervention_programs/trusted_private_target_boundary/private_target_rank`规定仅五个物理干预程序在私有侧以作者资产∩当前真实metadata position沿原匿名几何次序选目标；`minimum_qualified_visible_assets_for_replace/minimum_mask_pixels`为REPLACE至少2件/每件196像素；`asset_visibility_shortage_policy`记原slot失败，不回绕/换房；`target_repeat_policy`只计重复。四个null字段分别留给RELINK可移动物理前提、静态作者资产生命周期语义、NOOP/BIND/SPLIT/MERGE的类型化非干预区域规则，以及L1 oracle实体/结构分流。输入固定两house/扫描/来源，输出待审规则，例子是墙+椅子+杯子同框时墙不进入物理干预目标、但仍可进入公开结构观察。它不是私有ID进入部署packet、不是已通过模拟器干预，也不是已开放新数据生成。

[v3私有物理干预目标纯函数](../src/vsmt/vm04_target_selection_v3.py)只消费五个发物理动作程序的`program/instance_masks/current_metadata_objects/authored_ids`及显式静态/RELINK口径，输出资格过滤后的1–2个私有ID，资格不足抛异常占原slot；同几何mask并列亦明确失败，不按私有ID排先后。NOOP/BIND/SPLIT/MERGE调用它会被拒绝，不能靠物理资产门删掉合法结构观察。它并未写入任何公开packet、DINO descriptor或候选。输入mask ID属于可信模拟器/生成器过程，在线方法进程不得调用此函数。例如墙在原几何首位、挂画第二、椅子第三，BIRTH允许静态时取挂画、排除静态时取椅子。它不是由公开视觉模型识别出的类，也不是已接入生成worker。

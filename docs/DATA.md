...
双门候选与判定含义见[METHOD](METHOD.md#sh-04-r2-双门空间证据候选proposed待用户选择)。它需要独立的新合同版本，旧`spatial-history-pair-v1`仍严格表示两世界、两候选；不以旧32项测试或SH-03回执认证新接口。这里先登记数据权限，不生成schema、场景、训练划分或新数组。
...
首个候选四世界家族若获准，将含16条不同控制分支，独立重放另计16次；这是数量建议，不是已授权生成预算。具体观察变体会影响记录数量，需连同时间/磁盘上限先固定。数据角色仅为已见工程开发；未来训练/验证/确认的家族数和划分另审，不使用当前公开设计做独立确认。

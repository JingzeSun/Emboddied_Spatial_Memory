...
SH-02失败轨迹只读诊断（已实现，导出pending）：`ops/spatial_history/contact_diagnose.py`读取已经保存的四条首次分支，核验原报告/receipt、原Git提交字节及全部已登记产物摘要，再输出 `results/spatial_history_contact_diagnostic_v1.json`。它解决原报告只有终点、无法判断推杆何时接触或离开物块的问题；例如输入一条2450步轨迹，输出49段控制末尾的实际机器人/物块位置及每段连续推杆接触的起止和相邻状态。这不是重新模拟、推断未记录接触或改变接触阈值，也不向模型开放这些特权数据。

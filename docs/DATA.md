...
**修订一：干预窗口必须是整段过渡，且有最小长度——否则"面壁一帧"就能满足条件。** 原提案只要求"涉及容器在窗口内逐帧不可见"，没有规定窗口长度。一个偷懒的规划器只要让 agent 转身面壁一帧、把所有干预一次做完、再转回来，就逐帧满足了条件。这在标签上没错，但**dormancy 机制永远不会被触发**：S0-01 的休眠规则按"连续应可见却未匹配的次数"计数，一帧窗口下没有任何实体来得及休眠，REACTIVATE 与 dormant 归属这一整套机制在数据里就不存在了。修订：窗口＝扫掠一最后一个视点到扫掠二第一个视点之间的**整段过渡**，其全部帧都必须满足不可见；并新增登记值 `minimum_window_frames`（null，待冻结），窗口短于它记 `intervention_window_unavailable`。它与 `dormancy_missed_opportunity_limit` 应一并考虑，但两者分别登记。
...
| `minimum_window_frames` | — | **新增**，建议与 dormancy 上限同量级 |

...
附带的机制修正（不改规则）：被挡边割开视点格时在连通分量内重选最近合格视点；执行放置失败时重新取当前生成点逐点偷看、尝试表失败也写进 provenance；Floor 不再算容器；触顶错误带步数。仍留在范围声明里的限制：8～10 个房间的 house 扫掠一本身超 2000 步，非空必失败，本次不改 `maximum_actions`；`add` 的物体来自未见池（多为冰箱与抽屉内容物），BIRTH 结果须按物体类型单列；重访视点与扫掠一视点相同，未变物体的 BIND 偏易。

...
VM-04的目标来源诊断新增仅供可信私有生成器使用的`author_asset_ids`：从冻结ProcTHOR源house的`objects`及递归`children`取ID，并与`walls/doors/windows/rooms`分开；再用当前`metadata.objects.position`核验真实有限位置，另列`moveable/pickupable`能力。输入私有房屋来源、当前私有mask ID与simulator metadata，输出私有`authored_asset`/`movable_asset`资格profile及公开匿名计数。例如墙在`metadata.objects`中有ID和position仍不属于作者物体；挂画属于作者物体但可能不可移动。它不进入公开packet/descriptor/候选，不把私有ID哈希成模型特征，也未批准作为新生成目标规则。旧v2 scan的72个top-2条目仅2个作者资产，不能把原`metadata.objects`交集解释成非建筑对象过滤；[根因审计](../results/vsmt_vm04_root_cause_audit_v1.json)、[同pose资格审计](../results/vsmt_vm04_target_visibility_audit_v1.json)和[匿名重复审计](../results/vsmt_vm04_target_repetition_audit_v1.json)各有来源/私有文件摘要链。服务器private profile保留36槽逐pose ID，三个公开报告只存匿名计数。

[v3目标边界提案](../configs/vsmt/vm04_target_boundary_proposal_v3.json)是新版本、关闭式合同，字段白话含义如下：`pose_source/private_asset_membership_may_affect_pose`规定仍用v2固定相机位姿且私有资产不得挑镜头；`trusted_private_target_boundary/private_target_rank`规定私有作者资产∩当前真实metadata position后沿原匿名几何次序选目标；`minimum_qualified_visible_assets_per_pose/minimum_mask_pixels`为2件/每件196像素；`asset_visibility_shortage_policy`记原slot失败，不回绕/换房；`target_repeat_policy`只计重复。三个null字段分别留给RELINK可移动物理前提、静态作者资产的生命周期语义、L1 oracle实体/结构分流。输入固定两house/扫描/来源，输出待审规则，例子是墙+椅子+杯子同框时不让墙进入物理目标、但仍允许公开结构区域记录墙。它不是私有ID进入部署packet、不是已通过模拟器干预，也不是已开放新数据生成。

[v3私有目标纯函数](../src/vsmt/vm04_target_selection_v3.py)只消费`program/instance_masks/current_metadata_objects/authored_ids`及显式静态/RELINK口径，输出资格过滤后的1–2个私有ID，资格不足抛异常占原slot；同几何mask并列亦明确失败，不按私有ID排先后。它并未写入任何公开packet、DINO descriptor或候选。输入mask ID属于可信模拟器/生成器过程，在线方法进程不得调用此函数。例如墙在原几何首位、挂画第二、椅子第三，BIRTH允许静态时取挂画、排除静态时取椅子。它不是由公开视觉模型识别出的类，也不是已接入生成worker。

...
- 2026-09-11 用户补充本机 CPU 有问题，授权转服务器检查并继续采用由用户运行命令的方式。停止在本机重试；本机九项成功记录只作历史证据，不认证当前版本。新增 ROLE-S1/S2 服务器入口重新运行这九项以及原三十项 A–F 回归（旧测试自行生成少量 train/validation 夹具，不读取 S5/test 数据），独立记录服务器源码、环境、日志和退出状态。服务器两组检查均通过前，不启动新训练。本条属于同一原型工程阶段的验收环境修正，不是新科学方法或预算变化。

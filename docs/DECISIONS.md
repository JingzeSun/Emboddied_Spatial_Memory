...
- **为何不用旧 raw writer：** 旧 `vm04_multiview_raw.py` 会按 D-206 的2%声明噪声生成相对pose，与D-210撤销人为noisy-grid地点任务冲突。D-211 writer只保存RGB、depth、无pose相机内参和frame digest；provenance保存完整route与实际动作回执。它不输出world pose、instance masks、reference place、loop label、teacher、adapter packet或metric。动作失败保留成功观察前缀和失败动作回执后终止。

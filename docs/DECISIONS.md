...
- 立即工程修复：D-164配置转`blocked_invalid_v2_scan_targets`，探针执行位仍false；父入口在stage/controller创建前用冻结source record和private扫描目标验证资产归属，worker另设独立拒绝。旧v1/v2配置、扫描pose、原stage及失败字节不改。旧已完成16条只能作结构mask/流水线故障诊断，不能作为物体事务正例或效果样本；不会借现有20失败补样或根据private重复率换房。
...
- 独立[v3目标边界提案](../configs/vsmt/vm04_target_boundary_proposal_v3.json)与条件化验证器`0bef772`预登记原v2扫描receipt、固定pose/D、作者资产∩当前真实有限position、≥2个各≥196像素资格、按原匿名几何顺序选私有目标、重复只报告、资格不足占原slot失败；所有执行位关闭。待裁决的`relink_requires_moveable_or_pickupable`、`static_authored_asset_lifecycle_policy`、`l1_oracle_entity_structure_separation_policy`在待审态必须是null，未来只开探针的冻结态才要求显式口径。生成态尚未实现，不能靠切换状态开启episode；v1/v2/原stage不动。
- `83eddff`将上述两种静态资产口径做成需要显式参数的纯私有固定pose目标函数：允许生命周期临时隐藏静态资产时BIRTH可取排在前面的挂画；排除静态时改取可移动的椅子，RELINK在两口径下都只取可移动资产；SPLIT继续可用静态作者资产，REPLACE两目标按生命周期口径一起筛。资格不足抛原slot构造失败，不回落墙mask、换pose或靠private重复挑其他对象。它尚未接入v3生成/探针worker，不改变v2冻结扫描。

...
## D-116：先做E0像素相位因果归因，不先选择新数据协议

- 日期：2026-09-13。用户否决“先假定身体遮挡并据此重开数据”的推断，要求先研究清楚原因；用户允许后续轨迹包含相机x运动，但尚未批准具体横移轨迹、E0修订或数据重建。本职责只干预封存`r4-36`指定观察帧的相机x，世界XML、同帧积分快照、相机y/高度/朝向、传感器和E0数值全部固定。
- 原封存值的只读复算显示：LL/LR近门右边界投影为`u=58.003943790670576`，第58列射线落在墙竖直内侧面，派生高度`0.2997673988342284 m`。E0墙顶主簇区间为`[0.299899976158142,0.300099976158142] m`，该像素实际深度`1.1002326011657715 m`比“区间远端再加0.0002 m”的要求少约`0.0000674 m`；五行均被记为`gap_not_farther`。RL/RR对应边界为`u=68.95073336149915`且通过。遍历原121帧时LL/LR没有近门单帧候选，RL/RR有72帧。以上是诊断动机，不代替受控干预回执。
- 固定扫描为墙顶平面一个像素间距的`[-16/32,+16/32]`，共33个含零横移相位；原80像素、42度俯视条件下间距约10.56 mm。每个相位对4世界×2门只重渲染同一封存快照并运行原E0，共264帧、0次`mj_step`、0训练、0确认读取。公式的白话作用：把“横移多少米”换算成墙顶在图像中移动多少个像素，便于只改变亚像素对齐而不改观察距离。
- 公共阶段先保存全部深度观察和原E0输出，再封存摘要；随后私有审计才读取原XML目标及分割图，确认使E0拒绝的像素实际属于门墙、物块、推头或其他表面。零相位必须逐值复现原指定帧及原E0结果，非零相位不得改变时间、身体状态或相机其他自由度；所有源文件前后摘要必须相同。
- 白话：这个诊断解决“失败到底来自身体挡住、墙边恰落在像素缝上，还是E0本身不稳”的问题。输入是已经失败且封存的`r4-36`快照，输出是横移相位、具体阻挡像素和E0通过/失败的对应表。例如只移动相机约半个像素距离后门墙和身体完全不变，若E0由失败变为通过，就支持像素相位因果解释。它不是新训练样本、不是新观察轨迹，也不批准按结果挑一个横移量继续SH-05。

## D-117：E0-v2以边界兼容带扩大墙区间，不把未知补成自由

- 日期：2026-09-13。用户在D-116结果后批准三项语义：边界兼容像素只扩大墙边界区间且不认证自由；区间有效但最不利净宽不足时仍运行悲观M并正常计错；没有有序边界、缺严格更远中央证据或公共证据冲突时保持E0未决并在动力学前硬停止。E0-v1、r4-36 no-go、未知按占据和确认封存均不改写。
- E0-v2不增加结果调节阈值。原`minimum_farther_depth_separation_m=0.0002`保持；一个有效像素若深度大于墙顶高度区间的最远射线交点、但小于严格更远阈值，只记B（boundary-compatible，边界兼容），严格更远记F。两墙顶run之间仅接受`B* F+ B*`，前缘接受“墙顶—B*—F”；B若位于中央、没有F、伴随无效或非边界较近像素，均不能形成该观察的候选。
- 无B时边界保持v1的相邻墙顶/F完整足迹区间；有B时每个B按实际公开深度投影完整像素足迹，多枚B必须共享非空竖直面坐标区间，并与墙顶到最近F的宽转换包络相交；B不进入自由集。输出新版本`public-openings-v2`并保存B/F证据计数与每条支持的B像素。白话：输入仍是公开深度和标定，输出是有完整像素覆盖且不取像素中心的可能墙边；例如右墙面正好命中一列时用该列整个视锥约束右边界，而门中央仍必须另有F。它不是把这一列说成空地、不是降低0.2 mm，也不是用私有分割确认它属于墙。
- 当前只实现独立模块、审查配置和不含r4-36数值的人工正反例，状态`review pending`。代码审查后才允许新目录运行既有33相位公开扫描；该结果封存后才能进行私有归因评价，再决定是否按固定清单检查全部开发/验证家族。现阶段不接入M、不重建数据、不训练、不读取确认集；任何后续失败保留且不按结果修改B/F规则。

## D-118：复用D-116封存像素开展E0-v2固定相位检查

- 日期：2026-09-13。用户在审阅D-117实现说明和42/42服务器人工检查后回复“继续”，据此视为批准`e5bff8a..fe71e45`并授权下一项r4-36固定相位检查；授权不延伸到全开发/验证、双M、数据重建、训练或确认。新阶段必须使用独立目录，不覆盖D-116或原生成失败。
- 公共阶段逐摘要复用D-116的264个已封存RGBD观察，不重新渲染，只把每项`observation`交给E0-v2并同时保存原v1预测；全部新公开输出封存后才允许读取原XML、快照和分割。私有阶段重渲染仅用于核对封存观察与B像素geom，0物理步。白话：这解决“算法版本比较是否用了完全同一张图”的问题；输入是旧相位扫描的不可变像素，输出逐项v1/v2差异和B实际命中对象。例如原v1成功的一帧必须在v2得到逐值相同门区间。它不是生成第二套相位数据，也不让私有geom帮助v2判断。
- 运行前固定通过门：v1封存事实仍为262通过/2失败；v2必须264/264通过；原262通过项不能依赖候选B且去掉新增溯源字段后候选几何逐值相同；原2失败项必须含B支持且通过；全部候选B只命中对应登记门左右墙，物块/推头为0；坐标仍通过原25 mm区间/真值包含验收。任何不满足均正式停止并保留，不回调0.2 mm或B/F模式。

## D-119：固定相位v2首目录保留EGL环境失败，以显式后端新目录重开

- 日期：2026-09-13。`278ae70`服务器check为45/45通过，随后首运行完成264项公共复用与`public_seal.json`，但在私有阶段创建第一个MuJoCo renderer前因交互shell没有`MUJOCO_GL`而以OpenGL context错误退出；失败文件记录14.6199秒，0私有分割、0物理步，目录`sh05-r4-e0-boundary-phase-v2`原样保留且不得续写。
- 修复只在任何MuJoCo导入前固定`MUJOCO_GL=egl / PYOPENGL_PLATFORM=egl`并由入口核验，配置改用独立`sh05-r4-e0-boundary-phase-v2-egl`目录；E0-v2、B/F语义、264项输入和通过门均不改。白话：这是让无显示器服务器选择已登记的离屏渲染后端，输入输出图像语义不变；它不是换相机、补数据或放宽失败条件。新绑定必须重新check后才可run。

## D-120：50个封存开发家族的E0-v2固定准入评价

- 日期：2026-09-13。用户批准以现有50个已封存公开输入家族评价E0-v2：4个原工程家族、44个生成阶段已验证家族、`r4-22`和`r4-36`。`r4-22`仅提供E0证据，不因本评价成为可训练家族；硬停止时未封存的`r4-23/r4-58`明确排除并保留原目录。评价不补生成、不接M、不训练，也不读取confirmation。
- 公共阶段固定50×4世界×4模式=800项，从每家族`public_manifest.json`绑定的121帧RGBD重算E0-v1与E0-v2；所有输出写完并封存后，私有阶段才读取原E0-v1预测、family result、XML、快照与分割。原v1的800项必须逐值复现，预登记事实为796通过/4失败；否则输入或实现已漂移，阶段失败。
- 通过门固定为：v2须800/800通过；原796项不得退化；原4项失败须以非空B支持恢复；没有B的原成功项候选几何须逐值不变；全部B像素须命中登记门墙且物块/推头命中为0；原25 mm坐标区间上限不变。B的数量不预设，避免按结果追一个特定计数。任一门失败即保留no-go，不以删家族、补自由或改阈值续跑。
- 白话：这解决局部33相位修复能否推广到现有开发/验证数据的问题。输入是已经封存的50个公开历史，输出是800项逐例v1/v2结果和只在封存后进行的私有边界归因。例如`r4-36/LL/full`可以由真实门墙B恢复，但另一个原成功家族若被v2弄坏，整体仍失败。它不证明E0-v2已获正式前端资格；通过后仍须用户根据报告单独批准协议升级和剩余数据处理。

## D-121：E0-v2升级为新开发数据版本，观察仍为固定x的一维y往返

- 日期：2026-09-13。用户批准D-120建议并要求先继续运行：新版本只把E0-v2升级为公开几何前端，物理、控制、家族、划分、121帧时钟、80×80 RGBD和原观察轨迹均不改。相机在每个家族的全局x平移位置上保持x不随时间变化，只沿原y轴往返；不增加时变x横移。旧E0-v1源码/结果、`r4-36`正式no-go和两个被父阶段中断的目录原样保留。
- 52个开发/验证家族固定为原清单。已有50个完整源家族逐manifest、原始数组CRC/摘要、轨迹、重放和非E0门重新验证后只由新`dataset_index.json`引用，不复制或覆盖；`r4-36`只允许原`public_geometry`一项由D-120封存的16/16个E0-v2结果替代，任何物理、replay、信息或标签门失败都不能替代。`r4-23/r4-58`从空的新目录按同一旧轨迹和E0-v2各生成4历史、36主分支及36反序重放。白话：输入是50份不可变完整数据和两项未完成登记，输出一个52家族新版本索引及两份新原始数据；例如`r4-36`的新资格来自同一RGBD上的原则性E0-v2，不是改它的像素或删除它。它不是全量重渲染、样本替换或确认集释放。
- 新生成器对E0版本作显式派发：只有固定`sh05-r4-e0-boundary-v2-review-v1`调用`public_geometry_v2`，其余旧配置继续调用E0-v1；没有字符串匹配之外的自动择优。阶段在所有公共E0-v2输出封存后才作新协议私有评价；0训练、0权重下载、0 confirmation。双M和学习仍须消费新索引的后续独立入口和授权，不能从本条自动启动。
- 用户另行预登记升级条件：若完整SH-05结束后DreamerV3适配D、FloWM适配F、DINO-WM适配W三者都满足同一套在首个主训练结果产生前锁定的“表现良好”数值规则，下一数据协议版本加入时变x与y的二维观察轨迹；当前一维版本的分母、失败和结果永久保留，不能回写。该条件现在不自动释放新数据，具体“表现良好”数值仍须在训练前冻结，不能看SH-05结果后定义。白话：它解决“当前任务若对三个模型都太容易，下一版如何增加外部效度”的事前处置；输入完整SH-05的充分训练和统一评分，输出是否另建二维观察版本。例如三者都过门才讨论xy，而不是某个模型单次验证好就改当前数据。它不表示当前实验无效，也不授权现在增加x移动。

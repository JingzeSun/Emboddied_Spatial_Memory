...
- 边界区间跨越墙顶像素、连续B带和最近严格F的完整足迹；B不进入自由集。输出新版本`public-openings-v2`并保存B/F证据计数与每条支持的B像素。白话：输入仍是公开深度和标定，输出是更宽但有来源的可能墙边；例如右墙面正好命中一列时把这一列包进右边界区间，而门中央仍必须另有F。它不是把这一列说成空地、不是降低0.2 mm，也不是用私有分割确认它属于墙。

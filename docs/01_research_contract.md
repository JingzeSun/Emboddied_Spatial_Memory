...
D-059 同日进一步允许数据审查后彻底重构新 M1：先确定真实观测与旧记忆的来源合同，再逐模块审查实现和独立验证，之后进入 M2/M3。旧 M1 的历史具体设定不自动约束新协议，但旧失败仍保留；唯一科学主张、确定性版本化执行、在线不读未来及 C/E 主对照保持。具体流程复用 M1_V2_CLOSEOUT_FLOW.md，用户审查科学代码后再推进，不在本条预先确定新数据、架构或通过门。

...
**当前为SH-04-R2场景草案审查，待用户决定具体场景与任务判定；v1仍暂停实施（D-068）。** R1的[定向文献核查](../literature/notes/spatial_world_models_2026.md)已交付89e4ace。本轮形成[METHOD双门候选](METHOD.md#sh-04-r2-双门空间证据候选proposed待用户选择)和[DATA权限建议](DATA.md#sh-04-r2-四世界家族字段建议proposed未实现)：两门分开观察、四世界与四共同候选，分别审计后果歧义和动作价值。用户要求具体场景及判定由其拍板；当前只交设计示意，未冻结场景、阈值或预算。分支仍为`review/spatial-history-baselines`，main保持已审SH-03批次e901beb；科学代码、权重、生成数据和训练均未新增。
...
| SH-04-R2 场景可识别性合同（当前草案待选择） | 双门/四世界/四控制作为互补信息候选；先请用户决定是否先聚焦跨视角整合，以及用真实通行到达而非任意接触作为任务成败 | 选定后再交具体几何、连续相机路径、全部控制、工程清单/预算与容差含义，用户审后实现并做服务器验证。工程通过后另审连续几何—控制组合，不能用四类查表当最终研究任务 |

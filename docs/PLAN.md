...
**D-183审查修订（proposed、未并入D-182）：** [修订提案](../configs/vsmt/vm04_observation_suitability_d183_amendment_proposal_v1.json)采纳Claude指出的逐program聚合风险和SPLIT/MERGE事后贴标签风险，但不借此暗改D-182。建议pilot前封存至少70个合格house的固定顺序：pilot完成5–6个则正式N=48，完成4个则N=64，0–3个停止并另立版本；pilot只提供每family构造完成布尔，不能读取CFO/history/oracle结果。九个program必须分别报告CFO/history/gap；CFO超过冻结60%上限的program保留在总门分母中，但标为easy class且不得单独支撑该原子主张。SPLIT/MERGE只能由事前登记的几何、相机路线和冻结公开前端产生可复验伪影，未实现就记构造失败，不得换标签。该提案的精确几何、前端伪影判据和重复次数仍未冻结，且全部运行位为false。

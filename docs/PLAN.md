...
**当前SH-05在`r4-36`的E0公开几何门硬停止，先执行D-116像素相位因果归因。** 双M效果、三模型拟合和确认均未开始；原失败目录保持只读。用户要求先研究清楚原因，允许未来方案考虑相机x运动，但尚未批准具体轨迹、E0修订或数据重建。D-116只对同一封存快照扫描33个相机x相位，保持世界、相机其余自由度和原E0不变；服务器归因结果返回前不选择新数据协议。

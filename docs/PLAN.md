...
| 项 | 内容 |
|---|---|
| 状态 | 首跑 `159654f`：0.378、门未过且 add 真值缺失（LOG-236／237）。裁决 25～32 后在 `4bff1a8` 重生成（LOG-238）：46 house 成功 30（空窗口 10、含干预 20）、失败 16，非空成品率 0.556，**门 0.6 仍未过**；78/78 个干预可辨；墙钟 72 分钟。中间一次 `0bfbbc2` 重生成被编排器 bug 中止，整份保留。规模裁决 33 待定 |
...
| 继续门 | test manifest 此后只读一次；**裁决 39 必须先裁**（见下） |

**S3-01 开工前必须先裁的一项：dry-run 枚举上限（裁决 39，proposed，用户尚未裁决）。** 裁决 35 把 U 从 188 放大到 670 之后，dry-run 的开销是 `候选物体 × |U| × 最多 32 个点`，每个点都要真放一次再瞬移偷看一次。S1 的实测：99 个容器的 01451 单条就要一个多小时，50 条合计数小时。**300 个 house 的 S3-01 按同一口径约需 30 小时以上**，这是它必须在生成前裁掉的原因，不是可以边跑边定的工程细节。

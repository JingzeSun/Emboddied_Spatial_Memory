...
**当前为SH-04-R2具体工程数值审查；用户已认可双门场景和实际通过送达任务（D-069）。** 概念草案a134b1a获认可后，本轮交付[METHOD完整数值方案](METHOD.md#sh04-r2-two-gate)、[DATA字段权限](DATA.md#sh-04-r2-四世界家族字段建议proposed未实现)与[待审参数JSON](../configs/spatial_history/two_gate_engineering_proposal_v1.json)：门间1.60 m、局部俯视扫描、20 s固定四控制、目标完整容纳/末1 s低速及逐单帧信息损失门。场景方向不再重复请求许可；本次明确请用户决定有限视场观察条件与数值判定/工程预算，当前不可生成或训练。分支仍为`review/spatial-history-baselines`，main保持已审SH-03批次e901beb；未新增科学实现或实验结果。R1文献核查89e4ace与旧v1暂停状态D-068保留。
...
| SH-04-R2 场景可识别性合同（具体数值待审） | 已认可双门/四世界/四控制与真实送达；当前交完整几何、相机路径、控制表、任务/工程门和16+16执行预算 | 用户确认数值后按单一职责交新合同、独立物理适配和审计/导出；必要工程测试在服务器。全部入口一次交付、同步一次，满足依赖后顺序执行，不为转步骤再pull。工程通过后仍须公开几何正向对照，正式连续组合另审 |
...
R2本次**预算建议，未授权执行**：1个工程家族、4世界、每世界1条真实历史、16条首次控制分支及16次新实例反序重放；0训练步、0新权重下载，复用旧隔离环境。新数据上限2 GiB、生成墙钟上限30分钟，前台逐分支显示进度。与旧SH-03的物理步数和帧数为相近量级，但阻塞接触的可见性渲染可能增加开销；耗时未实测，30分钟是上限而非承诺。

拟使用全新数据盘目录`/root/autodl-tmp/spatial-history/sh04-r2-two-gate-engineering-v1`和导出报告`results/spatial_history_two_gate_engineering_v1.json`，当前未创建。数值/代码审查与所需测试回执满足后才生成；在完整分支后封存退出/摘要，再启动下一条。完整物理/信息审计失败仍保存固定清单全部结果，运行异常或预算到达则停止并标清未运行项。限制监控不得依靠只在末尾du；写入和分支间持续核验字节数/墙钟，超限保留部分现场，不覆盖、不自动重试/补样。独立重放的成功不是新增训练样本。

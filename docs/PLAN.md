...
**当前D-121已批准E0-v2升级为新开发数据版本前端；提交`9ce0a66..24b98d7`的服务器零模拟检查23/23通过，暂停在用户代码审查后释放实际补生成。** 新版本固定复用50个完整源家族并只重建中断的`r4-23/r4-58`；相机继续沿每家族固定x位置作原y轴往返，不新增时变x。旧E0-v1、`r4-36` no-go及所有失败目录只读保留；入口不读12个confirmation、不运行双M或训练。若完整SH-05后D/F/W三者均满足训练前锁定的同一“表现良好”门，才在下一数据版本加入时变x/y观察；当前分母和结果不得回写。详见LOG-137。

此前R4-4三模型工程接通已验收，R4-5学习合同与资源工程证据仍有效。用户选择DINO-WM替代PointWorld，当前三条为DreamerV3(D)、FloWM(F)、DINO-WM(W)。D16/W17/F19项完整121历史/200未来及反向传播全部通过，分别回传6a150e2/5813ff5/3167fd9；W公开DINOv2严格加载，原计算与缓存的值/梯度对拍通过，用户无需另行下载。D-096真实接口报告209bb34已核验：原清单首条公开历史、27模型-候选/162原始推演/200步全部完整，33证据和57原源码绑定一致，0训练/0真值读取；LOG-127明确engineering_ready=true、trained_model_ready=false。

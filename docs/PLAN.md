...
当前执行点：**S0-01～S0-03 已审通过（S0-03 返工版经 D-224-R 复审通过并提交）；S0-04 已实现待审，六项口径已由 D-224-LQ 裁决；S0-05 进行中**（LOG-214～LOG-218）。D-224 与 D-224-E/F/G 已批准，S0-02～S4 未开始；旧方向已归档到 `archive/pre-d224-unified-graph`，`main` 只含精简版文档。当前没有任何数据生成、训练或服务器运行授权。
...
| 输出 | `configs/vsmt/lean_s0_arms_v1.json` |

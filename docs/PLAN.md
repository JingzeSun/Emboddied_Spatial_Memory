...
**当前为R4-3a公共反投影/覆盖检索实现及完整服务器检查入口已交付（D-086，代码待审）。** 用户在D-085审议后要求继续；本轮沿既有公共传感器与2+8/0.02 m规则交纯深度值函数、19项科学人工例及8项运维检查，共27项尚未在服务器运行。具体合同见METHOD/DATA，代码事实见LOG-117。下一步审查本职责并运行下方一次同步的短检查；成功回传只认证人工覆盖值合同，不证明16真实历史检索或模型有效。D-084原7a2005c生成及33/37项回执已验收，原16历史/144首次/144重放不重跑。对象/地图/控制的数值仍先各自审议；其余60家族、确认、学习、下载及合并main仍未开放。旧失败与原PLAN局部编辑保留。
...
|---|---|---|
| R4-3a 公共反投影与覆盖检索（D-086已交代码） | `r4_coverage.py`深度专用值函数、19项手工检查，另8项运维检查及固定run/verify/export；不依赖对象拟合/地图/模型 | 原生80像素与轴向深度、裁剪、负坐标floor、去重、2+8贪心及并列/零新增、近期/前缀隔离、原时间/像素来源。用户审过该代码及同版27项必要回执后继续，不以选中10帧推定恢复两门 |
...
D-085本身没有新测试命令或计算预算；D-086仅细化a的下方标准库工程检查。后续实际恢复/近似动力学耗时记入另审工程预算，不记成原模拟重新执行，亦不把D-079待审总CPU额度当已授权余额。R4-4还需适配80像素的编码器输出（原四层stride2在80输入下不是原4×4）、九候选曝光和PointWorld资产；R4-3不私自决定resize/pooling或扩大训练预算。剩余非确认家族的生成放行仍须另核原清单、资源和依赖，不因R4-3a通过自动展开。

### R4-3a固定服务器检查阶段（D-086，27项未运行）

...
**D-172当前指针：原失败RELINK槽已复现碰撞拒绝与物理暂停差异，现复核手动物理结算。** 原两房36槽v1报告及D-171三控制stage均封存；同请求不强制传送被模拟器明确以传送后对象碰撞拒绝，正常强制动作即时重现x−4.9812 cm/z−1.4230 cm且isMoving=true，暂停物理的强制动作即时及后8帧三轴均精确到点。新增两个固定原房/pose/资产/请求复本，在暂停物理后各执行50个0.01秒手动物理步以判别碰撞后的结算位移；先做合同/纯检查/服务器资源与旧摘要核验，新stage只写private轨迹并匿名导出。AABB值不可用、机器人实际可达路径与旧记忆历史仍未核验；D-170 v2动作worker、完整生成和训练继续关闭。

...
**VM-04固定槽原始数据服务器批次（D-176/D-180，已暂停、服务器未运行）：** 代码仍固定原两房36槽、失败全留和四个RELINK缺口；生命周期门现要求`DisableObject`即时隐藏、disabled期每个注册相机event持续隐藏，并在`EnableObject`后的下一注册相机event确认重现；失败均保留私有逐帧证据。但原32帧只有交替±0.25°原地yaw、无平移，NOOP/BIND/SPLIT/MERGE没有已实现的公开类型化结构语义，四个RELINK全为预登记失败，且只有两间开发house。故即使36/36写出raw，也只能证明writer、公私隔离、摘要链、失败保留和资源派发，不能进入L2主表或支持VSMT优于适配器/朴素基线。配置现为`run_authorized=false/generation_authorized=false/expected_reviewed_code=null`；服务器0新episode。用户已认可不运行该批。D-182新观察合同的设计与数值现只为精确schema/实现审查冻结；共享probe的精确结构/训练预算、多视角终端重现窗口、visibility builder、SPLIT/MERGE构造及materializer receipt仍未实现，未开放生成。
...
**D-183规则已批准并进入精确schema/实现审查，执行关闭：** [基础合同](../configs/vsmt/vm04_observation_suitability_proposal_v1.json)已合入70-house固定顺序、6-family pilot的48/64/停止离散规则、逐program easy-class报告和禁止SPLIT/MERGE事后贴标签；[原修订记录](../configs/vsmt/vm04_observation_suitability_d183_amendment_proposal_v1.json)标为已批准审查但pilot/formal/training/confirmation仍false。[观察构造schema](../schemas/vsmt_vm04_observation_construction.schema.json)与[纯runner核心](../src/vsmt/vm04_observation_runner.py)现可确定性封存来源池、编译正式前缀、核24步/0.5 m/30°路线、公开不可观测干预和连续终端重现；[公开路线构造器](../src/vsmt/vm04_public_route_builder.py)已从干预前匿名pose扫描和注册动作图确定性搜索最短合格路线，并拒绝未来/动作结果或私有ID；[多视角worker核心](../ops/vsmt/vm04_multiview_worker.py)已按“初始Teleport观测0、每个后续动作新增一个观测”执行，公开回调只能看到剥离私有字段的RGB-D/相机记录，失败保留前缀且不换路。[stage入口](../ops/vsmt/vm04_observation_stage.py)仍只开放只读`check`。下一实现职责是可信raw/public/private落盘与materializer receipt、精确动作请求模板；SPLIT/MERGE精确几何/前端伪影判据、fresh replay次数及共享probe结构/预算继续为null，所以尚不能生成。

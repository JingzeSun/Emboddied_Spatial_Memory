...
**当前已完成SH-04-R3/E0服务器运行与只读结果审查，下一项为SH-04-R4共同接口、三份模型适配合同及公平对照/数据/评分冻结。** E0结果提交`3f25127`绑定运行源码`8f27a0e`：97项必要服务器检查和4世界×full/view_a/view_b/recent共16项真实查询全部通过，完整历史恢复两道门、A/B单段各恢复对应一道门、近期2帧不恢复门；来源、回执、嵌入产物、500帧消费、几何误差及资源门已静态核验，详见LOG-110。该结果只建立当前理想俯视家族的公开几何正向参照，不是DreamerV3、FloWM或PointWorld结果，也不证明动力学、历史利用或动作选择。下一批先冻结共同外层接口、三模型各自原生状态读取位置、长历史预测器/合法检索/地图加简单动力学主对照、新家族划分与预算；代码审过后才运行E1–E4或效果实验。当前仍在`review/spatial-history-baselines`，未合并main。原R2/R3产物与失败现场、R1文献核查及旧v1暂停状态均保持不变。
...
D-075诊断实验包括E0公开几何恢复、E1编码/保留读出、E2同控制单门配对、E3全时域推演、E4成功读出与选择。方法见[METHOD](METHOD.md#history-use-diagnostic)，字段见DATA。E0现使用D-078[并行活动配置](../configs/spatial_history/public_geometry_parallel_v1.json)，原D-077串行配置与D-076[数值提案](../configs/spatial_history/public_geometry_proposal_v1.json)保持原字节，分别核验运维变更和科学数值来源。E0通过只能提供公开几何可读的正向参照，不能直接判断模型在哪里丢失或没有利用信息。三条模型适配合同仍需明确原生状态读取位置，不能用真值补齐缺失状态；E1–E4探针学习和主模型效果等各自代码/家族/预算审过后再运行，当前四世界不用于拟合或独立确认。

...
**当前为R4-c/d审计流程完成、实际前端阻塞的原证据取回阶段。** c3aaae2报告绑定原41a9936：22项源码、36项检查、320项原输入及16历史/144主分支完整，run约1548.21 s；但144/144均perception_unresolved，名义完整预测及任务读出均0，尚未进入动力学，不能将R4-3e审计完成解释为地图/动力学验证可用。16条历史近期两帧均为1个合格候选加6个未排除分量；墙格全世界召回率仅7.15%–9.54%，精确率1不证明完整恢复。证据见LOG-119回传补充。下一步只取回已封存地图、原公开历史和独立评估XML，检查分量身份及墙面遗漏原因；不重新分割/建图/推演、不改判据，具体修订待证据。新导出入口及短命令见下方。原a/b、v2与本批成功工程证据按原版本复用；formal_model_ready与eligible_for_P仍false，R4-4、其余60家族、确认、训练、下载与main合并仍未开放。原PLAN局部编辑保留。
...
| R4-3e 本批独立工程审计（流程完成，预测链路未通） | c3aaae2回传原41a9936，36项检查及320原输入/16历史/144分支审计完整 | 144/144感知未决、0名义完整预测，尚无轨迹/接触/动作选择误差；先取回已封存分量与地图原证据，不重跑或放宽判据。LOG-119 |
...
原文件不写入；报告上限32 MiB，原地图与公开输入压缩字节共10,582,109 bytes，预计报告约14 MiB。前台，300 s/512 MiB，0新模拟/训练/下载/分割/建图/预测。完全相同的已有报告只核验复用，不覆盖；不完整或不同内容拒绝并保留。实际导出尚未运行，末行保存本次终检退出0并显示耗时。报告回传后先核字节，再做只读归因，判据修订仍待审。

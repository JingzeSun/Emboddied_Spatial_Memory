...
**步骤 01 已完成（只读清点，2026-09-11）；步骤 02 等待数据访问。** [基线清单](../data/manifests/m1_rebuild_baseline.json)绑定旧提交 `5c5450b64e780fee33fd971de1cfe713125495ed`，登记 167 个源码/运行配置等文件、30 个函数位置及 43 个本地报告的文件哈希；[旧代码阅读地图](METHOD.md#旧代码阅读地图步骤-01)说明待替换职责。这是定位与完整性记录，没有重算实验指标，也未核验服务器 outputs。清单采用 Git blob 字节计算源码摘要，不能替换旧运行按各自规则保存的 source hash。

用户确认尚未申请或仍在等待 3RScan 访问。已核对的公开训练索引包含 385 个不同 scan ID，但尚不能据此确定同场所及重访分组；未读取 validation/test 索引，未下载原始 RGB-D，未选定场所。下一项实际前提是通过[官方获取入口](https://waldjohannau.github.io/RIO/)取得下载方式及场所关联元数据，再登记一个训练场所的具体 scan ID、文件类型和来源。随后交付阶段 A 的固定服务器命令；本次文档/清单提交不要求服务器同步或重跑旧任务。R1 之前不固定新 M1 的架构、标签或题目。

...
本文件只维护 Versioned Structural Memory Transactions（VSMT，版本化结构记忆事务）从当前代码到论文结果的步骤链、依赖、状态、交付物和**失败分支**。第三至七章是理想路径，[第八章](#八失败分支与备选路径)为每个剩余步骤预先登记失败形态与备选路线——备选必须在该步骤运行前登记，事后发明的退路属于移动球门。决策依据写在 [DECISIONS.md](DECISIONS.md)，运行与证据写在 [EXECUTE.md](../EXECUTE.md)，方法定义写在 [METHOD.md](METHOD.md)，数据字段写在 [DATA.md](DATA.md)。历史路线和旧服务器命令不再堆在本文件；需要时从上述文件或 Git 历史查找。
...
前七章描述的是理想路径。本章为每个剩余步骤预先登记**失败长什么样、退到哪一条备选、退之后论文还能声称什么**，目的是让项目在任何一步不如预期时都能继续，而不是走到黑或临时拍脑袋改规则。

**两条使用规则。** 第一，备选路径必须在对应步骤**运行之前**就登记在此表中；看到不利结果后才发明的退路不是备选路径，是移动球门，必须作为新决策显式登记并标注 post-hoc。第二，选择备选路径不等于掩盖失败——原失败连同它的证据照常报告，备选只决定项目往哪走，不决定论文怎么说。

E-08 已经用过一次这套机制：审计证伪了"单帧可恢复结构角色"，于是 P08 的认证来源换成构造期拓扑规则（D-223），而审计结果本身照实进论文。那次的教训是当时**没有**预先登记退路，所以花了一整轮讨论才决定怎么走；本章就是为了不再发生这种事。

### F 阶段：共享前端与两房 smoke

| 步骤 | 失败长什么样 | 登记的备选 | 退之后还能声称什么 |
|---|---|---|---|
| F-01 production reader | SAM 每帧 proposal 过多或过少；冻结 DINO 描述子跨视角区分度不足，同一物体在两个视角 cosine 低于同场景不同物体 | 退到 **L1 oracle 轨道**：用隔离 materializer 产生的匿名真值 mask 作 proposal，其余（DINO、深度几何、候选、teacher、评价）完全不变 | 只能声称"排除检测误差后，五种记忆机制的能力与失败归因"，即 METHOD 的 L1 层；**不得进入 L2 主排名**，论文标题与摘要相应改为机制诊断 |
| F-02 P08 拓扑认证 | D-223 的拓扑规则在两间 P0 house 上找不到 basin→bottleneck→basin 路径 | 先查是否为 house 本身没有合适拓扑（换 house 属于新决策，需另审）；若确实没有，**放弃 P08 作为独立场景**，改为给 P01/P03/P05 之一**增加实体观测要求** | 实体挂载证据不必依赖"房间—走廊—房间"这个特定几何：任何"实体在 A 被观测、随后重访"的路线都能考察 entity-place attachment。退之后仍有实体证据，只是失去"跨房间结构一致修订"这一更强的构造 |
| F-02 P04 资格 | 冻结 DINO top-1 选出的 pair 视觉混淆强度不足，构不成有意义的感知别名负例 | 保留原 pair 并如实记录其绝对 cosine 分数；P04 降为 supporting，headline 由 P03/P06/P07 承担 | 仍可声称"在该数据上未构造出足够强的感知别名"，这是对构造难度的诚实报告，不是方法失败 |
| F-04 两房 raw | 路线执行中动作被拒、raw 写入不完整 | 保留失败前缀，减少可用槽位继续；两房本来就只是工程 smoke | 两房结果本就不得进入任何论文表，槽位减少不影响主张 |

### M 阶段：开发比较

| 步骤 | 失败长什么样 | 登记的备选 | 退之后还能声称什么 |
|---|---|---|---|
| M-01 统一图接线 | 非网格 place 的 BIND/BIRTH/MERGE 候选无法从 D-214 缓存构造 | 这是当前已知的最大未接通项。若几何证据确实不足以形成 place 候选，退到**只做 entity/surface/fragment 三类节点**的统一图，place 由确定性 scaffold 维护 | 论文的创新点 1 从"统一图涵盖地点"收缩为"统一图涵盖实体与表面"；地点修订退出主张，与 D-205 的旧口径一致 |
| M-03 五方法开发比较 | VSMT 在两间 house 上不优于甚至劣于基线 | **不构成任何结论**，两间 house 本就不足以选 headline 赢家；照常进入正式数据 | 无需声称任何东西；该阶段只用于发现工程与候选问题 |

### P 阶段：正式数据、训练与 test

| 步骤 | 失败长什么样 | 登记的备选 | 退之后还能声称什么 |
|---|---|---|---|
| P-02 正式 raw 生成 | 构造成品率远低于 P-01 预算假设，样本量不足以支撑预登记功效 | 按 P-01 已冻结的停止规则收口，用实际得到的样本量运行，并把功效不足写入限制章节 | 可声称在实际样本量下的结果与其置信区间；不得事后加样本，也不得因样本少而改主指标 |
| P-03 VSMT selector 训练 | 训练不收敛，或在预算内明显未充分训练 | 按已登记的 `training_sufficiency_unverified` 判据标注，照常进入 validation；同时报告 NECS 的结果 | 不得把训练不足写成"方法失败"；若 NECS 与 VSMT 相当，说明"执行候选后再比较"未获独立支持，这本身是有效发现 |
| P-05 test 主比较 | VSMT 在主指标上不优于 TAF/ELU/WFR/LOW | **如实报告 no-go**，不换数据、不缩减强对照 | 论文仍有四项可发表的贡献：同一冻结前端下的五方法可比协议；candidate-before-teacher 边界；candidate miss / teacher error / 摊销误差的分解；以及"显式版本化事务在该条件下未带来可测收益"这一阴性结果 |

### 全局备选：如果时间或资源不足以完成 L2 主比较

L2（所有方法共用冻结 RGB-D 前端）是理想目标。若 F-01 迟迟不可用或正式数据成本超出预算，**整篇论文可退到 L1 层**：用匿名化真值 mask 作 proposal，其余协议完全不变。这是 METHOD 已登记的三层证据结构中的中间层，不是临时发明的降级。

退到 L1 的代价必须写清楚：论文不能声称可部署 RGB-D 条件下的结论，只能声称"在感知正确的前提下，哪种记忆修订机制更可靠"。对一篇以记忆机制为创新点的论文，这仍是完整且可辩护的贡献。

...
- [ ] 实现九类program的公开前提构造与验收：NOOP/BIND及生命周期类须有对应公开旧记忆条件；SPLIT/MERGE只走上述预登记确定性几何；不能用旧静态worker的private target规则或事后标签。
  - D-199候选已实现九类结构前提槽和SPLIT/MERGE逐replay回执；D-200新增无默认值公开充分matcher、route receipt绑定及九类正反例，明确不证明身份真值。D-201候选把matcher receipt接到单episode materializer/causal/terminal packet摘要链，失败仍留construction failure，但matcher通过仍因缺在线plan时间收据而禁止进入family完成；正式matcher数值、edge RETRACT跨时公开关系缺席证据和父stage family聚合仍缺，故仍不勾选。
...
**当前用户交审拆分（按D-059停止继续堆叠）：** 用户已正式认可D-200；D-201现形成单episode matcher/materializer摘要绑定候选，采用terminal packet之前的causal prior，把matcher失败机械保留为construction failure，并让matcher通过继续阻断于在线plan时间封存，等待科学口径与代码审查。下一批不得越过仍为null的matcher/visibility/SAM数值、edge RETRACT证据缺口或未审D-201去实现family完成聚合或可运行父stage；批量派发、资源回执和摘要/导出也仍须另作单一职责交审。现有封装保持fail closed并保留测试。

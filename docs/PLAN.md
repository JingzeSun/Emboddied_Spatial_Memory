...
**当前为SH-04-R4规格审查：D-079已将共同接口、D/F/P三条独立适配、L/R/M强对照、新家族分组和评分/预算写成固定v1提案，尚未实现或运行。** [数值登记](../configs/spatial_history/protocol_r4_v1.json)为`frozen_for_review_not_executable`；[METHOD](METHOD.md#r4-common-contract)维护方法和三份合同，[DATA](DATA.md#r4-data)维护字段与64家族划分。新增数值是具体待审设计，不把本次开启阶段记为对未见代码、56 GPU小时或64 GiB新增磁盘的运行批准。下一职责为共同接口/评分器的独立可审实现与必要服务器检查；随后按下方R4顺序交新家族生成器、公共地图/控制模块、三条模型适配。E0审查a516395及结果范围见LOG-110，完成产物按原来源复用。当前仍在`review/spatial-history-baselines`，未合并main，旧test/no-go不变。
...
| R4-1 共同查询与评分 | 独立schema/适配器、训练标签分离、区间接触、候选选择/缺失分母、家族统计；解析正负例及完整check/verify/export | 用户审查本职责代码，服务器必要检查通过；不依赖尚未实现模型 |

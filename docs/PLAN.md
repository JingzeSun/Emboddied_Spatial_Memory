...
- [ ] 实现九类program的公开前提构造与验收：NOOP/BIND及生命周期类须有对应公开旧记忆条件；SPLIT/MERGE只走上述预登记确定性几何；不能用旧静态worker的private target规则或事后标签。
  - D-199候选已实现九类结构前提槽和SPLIT/MERGE逐replay回执；D-200新增无默认值公开充分matcher、route receipt绑定及九类正反例，明确不证明身份真值。正式matcher数值、edge RETRACT跨时公开关系缺席证据和父stage/materializer receipt接入仍缺，故仍不勾选。

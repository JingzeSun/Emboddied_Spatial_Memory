...
当前执行点：**S0-01～S0-03 已审通过（S0-03 返工版经 D-224-R 复审通过并提交）；S0-04 已实现待审，六项口径已由 D-224-LQ 裁决；S0-05 已实现待审，五项口径已由 D-224-SW 裁决；S0-06 用户合同审查进行中**（LOG-214～LOG-218）。D-224 与 D-224-E/F/G 已批准，S0-02～S4 未开始；旧方向已归档到 `archive/pre-d224-unified-graph`，`main` 只含精简版文档。当前没有任何数据生成、训练或服务器运行授权。
...
| 项 | 内容 |
|---|---|
| 状态 | **进行中**（2026-09-20 开始；S0-01～S0-03 已审通过，待审 S0-04 与 S0-05；跨合同一致性由 `test_vsmt_lean_cross_contract.py` 机器核对） |
...
|---|---|
| 状态 | 未开始 |
| 输入 | D-215 冻结的 SAM 2.1 / DINOv2 ViT-S/14 摘要；DINOv2 ViT-B/14 摘要待登记 |
| 完整动作 | 用户授权后下载并核对资产摘要；探测 CPU、RAM、GPU 显存、磁盘；按单 worker 实测占用定最大安全 worker 数 |
| 输出 | 资产回执、容量回执 |
| 继续门 | 摘要不符即停 |

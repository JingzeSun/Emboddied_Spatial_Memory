...
**D-173当前指针：两房原四个RELINK的固定x+0.5 m端点全部被非强制碰撞门拒绝，新生成继续阻断。** D-171/D-172已把原slot12的强制传送后态偏移归因为冲突端点的物理结算；D-173只改变四槽`forceAction`并重放原house/pose/资产/相机动作，匿名报告见LOG-162。原x+0.5 m端点规则不能作为可执行物理RELINK数据前提。下一职责先提出**独立于这四槽结果预登记**的可行端点/真实操作路径构造规则及记忆关系前后核验方案，给出可区分“端点碰撞、路径失败、记忆语义错误”的正反案例与代码审查；不得从碰撞结果反推新端点、换house/slot、靠强制传送接受假成功。D-170 v2动作worker、完整episode生成、训练和效果仍关闭。

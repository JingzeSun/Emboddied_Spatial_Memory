...
**2026-09-25 前后对照与 oracle 前端探针完成，提出待裁 72（LOG-257 续，`6cc0af5`／`03e8d20`）**：裁决 68 的去重与应可见新值在 SAM2 前端上只把 TAF 的 IoU F1 从 0.211 提到 0.224（每真值实体 3.52→3.19）；完美分割（模拟器实例 mask 经 `--masks-from` 建的 oracle cache）把它提到 0.399，并框上界 IoU 0.64／质心 0.86——SAM2 过分割与深度表面壳框各占一半，剩下的是记忆逻辑。**待裁 72**（方向简化，DECISIONS）：(A) 主表前端改用实例分割、SAM2 降附录；(B) 质心口径为主节点列（裁决 70 改取 (b)）；(C) 开发表七臂保留、S3 臂集另裁；(D) 停掉 SAM2 拟合趟、在 oracle cache 上重跑。dagger_round_0 按用户指示未启动。**下一步**：用户裁 72 → 若 (A)：S1-03／S0-03／S1-04 合同修订与重钉、39 条 oracle cache 重建、校准趟与拟合趟在 oracle cache 上重跑 → 后续四趟。

**2026-09-25 裁决 72 全按推荐批准（LOG-258）**：主表前端改为模拟器实例分割（SAM2 降附录）、质心口径为主节点列（裁决 70 改取 (b)）、开发表七臂保留、SAM2 拟合趟已停（4/39，根标记 STOPPED）。**当前执行点：裁决 72 落地（新对话）**——S1-03／S0-03／S1-04 合同修订与重钉、`lean_s1_03_cache.py` 实例分割模式、S0-04／S2-04／S2-05 主次节点列交换、METHOD／DATA／AGENTS 同步、39 条 oracle cache 重建、校准趟与拟合趟重跑；之后才是 dagger_round_0 → dagger_round_1 → development_table。

**2026-09-25 裁决 72 落地（LOG-259）**：四个单一职责提交——`9bb2c91` S1-03 合同 `mask_source` 块（实例分割主表／SAM2 附录，实例模式只读私有面五项、只出匿名 mask 几何）、episode 封印对非 SAM2 来源写入来源（SAM2 封印字节不变，`lean-s1-03-154776d` 不用重建）、S2-04／S2-05 入口必须声明 `--mask-source` 并拒绝另一来源的 cache，规则摘要 `c7318cd5…` → `f354863c…`；`942f45f` 生成器 `--mask-source simulator_instance_masks` 正式模式；`8ebbd05` 主次节点列交换（`node_prf1`＝质心 ≤ δ_moved 主列与选参指标，IoU 0.3 改名 `node_prf1_iou` 次级列），S0-04 `c5d66505…` → `b831a3a4…`、S2-04 `0ff1fbe9…` → `d1645593…`、S2-05 `0d3cf50c…` → `57dc753d…`；S0-03／S1-04／S0-05 只动记账与白话，摘要不变。服务器全量 `8ebbd05` **1618/1618**。**当前执行点：39 条 oracle cache 重建（`lean-s1-03-oracle-8ebbd05`）→ 校准趟 → 拟合趟**，之后每趟先报再启：dagger_round_0 → 训练第 0 轮 → dagger_round_1 → 训练第 1 轮 → development_table。待裁 73（ReID 头复用／重训、网格复核）等 oracle 校准分位数出来再提。

**2026-09-26 oracle cache 与校准趟完成（LOG-259 续、LOG-260）**：cache 39/39；校准趟 39/39（19,035 s），服务器按链脚本自动出报告、导出并关机。关联一侧分离变好（异物体余弦≥0.7 从 32% 降到 17%），应可见分位数不变；自由空间覆盖中位数从约 0.65 降到约 0.06，且仍分不开在场与消失。**当前执行点：待裁 73**（关联网格、去重、应可见沿用；RAC／HandCost／ELU-P 的覆盖阈值按新分位数重登记；ReID 头复用），裁决后下次开机跑拟合趟，同时复核 ReID 召回曲线。

**2026-09-26 裁决 73 批准“最好方案”**：(1) 关联网格、去重、应可见暂不动；(2) 存在证据改逐点深度检验，先做只读探针（`lean_s2_05_depth_probe.py`，16 条 × TAF／LOW）；(3) 在实例分割 cache 上重训 ReID 并按 S1-05 规则重选。**当前执行点：下次开机跑 `ops/vsmt/ruling73_boot.sh`**（全量测试 → 探针冒烟 → S1-04 `--reid` 与探针并行 → 导出 → 自动关机，约 1.5～2 小时）；随后按探针结果提出存在证据的合同修订，S1-05 重选并重钉 S0-03，然后重跑校准趟，再依次跑拟合、两轮 DAgger 与开发表。

**2026-09-26 裁决 73 开机完成（LOG-261）**：旧实例 01:13 CST 被外部停机，换克隆实例续跑后全部完成。ReID 重训重选仍为 `reid_projection:vitb14`，新权重已重钉（`87316cb`）；逐点深度检验拿到证据的行多约 4 倍，AUC 0.57 → 0.60～0.61。**当前执行点：待裁 74**（存在证据正式改为逐点深度检验，余量 0.05 m；校准收集器按 gone 原因拆列）。批准后实现合同与 runner 修订，再开机跑全量测试与校准趟。

**2026-09-26 裁决 74 批准并落地**：逐点深度检验（`e62670b`）与校准诊断拆列（`fd2042f`）。**当前执行点：开机跑 `ops/vsmt/ruling74_calibration.sh`**（全量测试 → 新规则下的校准趟 → 报告与导出 → 自动关机）；之后按新分位数提出应可见下限与 RAC／HandCost／ELU-P 覆盖阈值的裁决，再依次跑拟合、两轮 DAgger 与开发表。

**2026-09-26 裁决 75 批准并落地**：实体按自己的表面点检验（`c9682eb`），校准臂改为带门的 TAF 并顺带产出 ELU-P 计数（`e9e0d1e`），理想记忆证据上限诊断（`05e539c`）。253d6bd 的无门 LOW 校准趟跑完只作存档。**当前执行点：`ops/vsmt/ruling75_calibration.sh`**（全量测试 → 证据上限 → TAF 校准趟＋ELU-P 计数 → 拟合 → 导出 → 自动关机），在 253d6bd 那趟结束后于同一实例上接着跑；之后提出阈值裁决与 ELU-P 拟合量登记，再跑两轮 DAgger 与开发表。

**2026-09-26 GPT Astra 节点 F1 审查已逐条核实，提出待裁 76（LOG-262）**：TAF 质心列 98.5% 的错误是多出来的预测（重复新建卡在 θ，不在召回）；"SAM2 占一半""完美合并上界 0.64／0.86"两处表述已更正；待裁 76 四项——只读归因审计、去重资格含 dormant 与折叠取最新几何、节点匹配先数量后权、输入核验与分位数表述。**当前执行点不变**：等 ruling-75 趟导出，再提待裁 77（应可见下限、RAC／HandCost／ELU-P 阈值、去重三值复核、ELU-P 拟合量登记）。

**2026-09-26 裁决 76 批准并落地（LOG-263）**：去重资格含 dormant、合并取较新几何（S0-01 `76a00901`）；节点两列先最大匹配数（S0-04 `eb81780b`）；帧摘要重算、z ≥ 0.05、精确分位数；审计 v2。同 4 条 episode 上 TAF 质心 F1 0.515 → 0.526、LOW 0.761 → 0.769；原有 active–active 折叠约一半是不同物体，去重三值进待裁 77。**当前执行点**：服务器 `relay_74_to_76.sh` 等 253d6bd 趟结束后在 `d231474` 上跑 `ruling75_calibration.sh`（全量测试 → 证据上限 → TAF 校准＋ELU-P 计数 → 拟合 → 导出 → 自动关机）；之后写 LOG-264／265，提待裁 77。

**2026-09-26 提出待裁 77（LOG-263 续）**：节点主列改为"自己的物体且质心 ≤0.5 m 或在真值框外扩 0.25 m 内"（现行口径 TAF 18%、LOW 34% 的配对是别的物体）；共享去重余弦 0.8 → 0.6（TAF 身份＋框 F1 0.500 → 0.638）。**当前执行点**：服务器 `relay_gated.sh` 在 253d6bd 结束后取消其自动关机，等开跑标记；待裁 77 批准并落地后写入新提交号，ruling-75 趟开跑；阈值与 ELU-P 拟合量改由待裁 78 在校准趟后提出。

**2026-09-26 裁决 77 批准并落地**：节点主列须是自己的物体且质心 ≤0.5 m 或在真值框外扩 0.25 m 内（`91e936f`，S0-04 `8b57bacb`）；去重余弦 0.6（`4a59789`，S0-01 `15dbabb9`）；本地 lean 测试全过。**当前执行点**：开跑标记写入本提交，服务器 `relay_gated.sh` 在 253d6bd 结束后于该提交跑 `ruling75_calibration.sh`（全量测试 → 证据上限 → TAF 校准＋ELU-P 计数 → 拟合 → 导出 → 自动关机）；之后写 LOG-264（253d6bd）、LOG-265（本趟），提待裁 78（阈值与 ELU-P 拟合量）；开发表后复核学习臂在去重 0.6 下的误合并。

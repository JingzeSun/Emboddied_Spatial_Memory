...
| 项 | 内容 |
|---|---|
| 状态 | 已在 `159654f` 运行一次（46 house，4 worker，21 分钟）：成功 23、失败 23，非空成品率 0.378，**门未过**（LOG-236）；且 87/87 个 `add` 私有真值缺失（LOG-237）。修正代码已备好并 smoke 通过；规模裁决 25～32 待定，重生成未授权 |

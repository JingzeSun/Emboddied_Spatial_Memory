...
**当前D-116已确认`r4-36`为E0像素相位退化，暂停在人工语义选择。** 264次同快照重渲染中零相位精确复现LL/LR近门失败，阻挡像素均属于`gate_0_right`而非物块/推头；一个像素周期内其余32个x相位全部通过，详见LOG-133。该结果不自动授权横移轨迹、E0修订或数据重建。下一步须由用户决定把E0继续作为数据准入门、把本次视为M/E0基线失败，还是建立原则性边界像素处理的新E0版本；决定前双M效果、三模型拟合和确认均不开始，原失败目录保持只读。

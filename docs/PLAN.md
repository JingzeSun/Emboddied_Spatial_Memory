...
**当前D-118已获用户“继续”授权，正在交付E0-v2的r4-36固定33相位检查。** 公共阶段复用D-116逐摘要封存的264张相同RGBD并比较v1/v2，全部封存后私有阶段才核B像素geom。通过门预先固定为v2 264/264、原v1成功262项候选几何逐值不变、原2失败项均由B恢复、全部B只命中对应门墙且身体命中0；任一失败保持现场并停止。全开发/验证、双M、数据重建、三模型拟合和确认均未授权，E0-v1、LOG-133与原失败目录保持只读。

...
**S1-03 已实现待审（2026-09-21）**：合同、纯核心（36 项测试）与 runner 已提交，五个授权位全 false。实现时查出两处从未被冻结的值——自由空间可靠性门 `ρ_free`、`supported_by` 的五个几何阈值——都已登记为待冻结，runner 在它们为 null 时拒绝运行；另补上了冻结前端记录里缺失的三维包围盒（均值不是盒心，推不出盒子），并用测试钉住它与冻结 `extent_m` 一致。

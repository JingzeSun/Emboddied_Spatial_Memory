...
用户没有 3RScan 申请表要求的机构/导师信息，已要求考察其他数据集，不再把等待 3RScan 审批作为唯一下一步。[来源比较](DATA.md#公开实采接入的来源比较proposed延续-d-058d-059)已核查 Bonn、ARKitScenes、ADT 和 BEHAVE。D-061 恢复环境结构与对象共同审查：优先核查 ADT 公寓中的转向/跨区域连续片段，以及 ARKitScenes 单次扫描中墙地面逐渐揭示与重见的片段；Bonn 箱子序列降为对象变化补充，不再作为独立满足首批目标的方案。具体走廊/转角片段尚未确认，不能先承诺数据集已覆盖。这是样本考察建议，尚未正式选定新 M1/M2 来源，也未下载图像或交付服务器运行阶段。下一步先固定首批下载清单、来源/许可、开发用途、时间关联和缺失处理，再准备小样本检查命令。没有官方训练分组的来源不得伪称官方 train；先只作开发，关联场所全部排除后续未见确认。R1 前不固定新 M1 的架构、标签或题目；本次来源文档更新不要求服务器 pull 或重跑旧任务。

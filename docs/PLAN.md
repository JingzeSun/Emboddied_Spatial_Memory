...
**计划日期：2026-09-11。状态：用户已授权开始执行；步骤 01 的只读基线清点已完成，当前指针见文末。新科学模块和实验仍为 planned，除非 EXECUTE 提供实际实现或运行证据。**
...
输入前提：服务器尚无公开数据；最初优先考察 3RScan，用户现要求核查其他来源，候选比较见 DATA，正式来源尚未选定。输出是原始样本、数据来源表和人工审查案例。此阶段不写学习器、不比较 CTL 输赢。
...
| 04 做可由人判断的案例卡 | 先按固定均匀帧位置浏览，再记录旧墙/地面重见、结构范围扩展、转角/门口连接新区域，以及对象位置变化、遮挡/空位和身份歧义；每卡显示当时可见信息、后续证据和单独标注 | 每卡可追溯到 scan/frame；分别填写“能判断/不能判断/标注与画面冲突”，不把缺标注当消失，不把人工挑选案例当代表性发生率 |
...
| 07 定义旧 latent 与修订语义 | 确定结构片段与对象的节点表示、几何支持范围、连接关系、证据归属、生命周期和版本；用走廊扩展及对象变化各写 before/after，区分仅改引用、改几何、改描述向量 | 一次 BIND/RELINK/REPLACE/SPLIT/MERGE 的可读例子；新观测不默认新实体；不允许先把观测融合进旧向量再决定身份 |
...
**步骤 01 已完成（只读清点，2026-09-11）；步骤 02 核查替代来源。** [基线清单](../data/manifests/m1_rebuild_baseline.json)绑定旧提交 `5c5450b64e780fee33fd971de1cfe713125495ed`，登记 167 个源码/运行配置等文件、30 个函数位置及 43 个本地报告的文件哈希；[旧代码阅读地图](METHOD.md#旧代码阅读地图步骤-01)说明待替换职责。这是定位与完整性记录，没有重算实验指标，也未核验服务器 outputs。清单采用 Git blob 字节计算源码摘要，不能替换旧运行按各自规则保存的 source hash。

用户没有 3RScan 申请表要求的机构/导师信息，已要求考察其他数据集，不再把等待 3RScan 审批作为唯一下一步。按 D-061，优先核查 ADT 跨区域连续片段与 ARKitScenes 结构揭示片段，Bonn 仅作对象变化补充；M2 慢修订同时保留结构误接、重复片段与对象错误修复。

**步骤 02 已完成第一轮实际画面筛查，步骤 03 的连续性与几何验收仍待完成。** 已查看 ARKitScenes 官方 Training 视频 47333462 的 12 张原始 RGB 帧，以及 ADT 公寓 Cleaning/RoomDecoration 两段的全部 20 张官方预览缩略图；来源、摘要、选择过程与 6 个定性案例见 [审查清单](../data/manifests/visual_source_review.json)，用途和限制见 [DATA.md](DATA.md#首批画面核查开发预览)。本地审查页为 `outputs/data-source-review/index.html`。图像下载不等于完整原始数据接入；没有下载 ADT 原始 VRS、深度或位姿，也没有本地视频解码或科学计算。

下一步优先取得 ADT 装饰段的连续 RGB，检查穿门路径、旧/新结构共同锚点与相框搬放的时间次序；同时对 ARKitScenes 该段准备 RGB、深度、标定和位姿的服务器小样本一致性检查。首批完整下载的许可、文件清单、时间关联、缺失处理和预算须在交付固定阶段命令前落实。具体走廊拐角仍未确认；用户画面复核与 R1 数据验收尚未完成，不能先冻结 M1 架构、标签或题目。此次只作开发考察：ARKit visit 467138 及关联场所、两条 ADT 所在公寓均排除于后续未见场所确认；对象资产分组还需核查。来源记录和文档更新不要求服务器 pull 或重跑旧任务。

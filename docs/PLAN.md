...
**D-173当前指针：原slot12碰撞与强制传送后物理推离已由独立控制复现，现只核查两房原四个RELINK端点。** D-171同请求非强制传送被模拟器明确以对象碰撞拒绝；D-172两个独立暂停物理复本在首个0.01秒手动物理步都离开请求点，50步后达到与原强制动作同向、近同量级的偏移，根因是冲突端点被强制放置后物理结算。只在原四个固定RELINK槽上把原请求的`forceAction`改false、保留其他字节，并要求真实即时/终态位置在5 mm内且目标停止运动；失败原槽保留，不改house、pose、目标、端点或slot。先核原报告/source摘要、精确干净提交和资源，再按四个独立worker诊断并匿名导出；0新episode/训练/记忆正例。机器人实际操作路径与记忆历史语义仍未核验，D-170 v2动作worker及完整生成继续关闭。

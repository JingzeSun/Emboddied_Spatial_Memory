...
**D-170当前指针：固定两房动作探针已运行，v2资源预检仅供代码审查。** [匿名v1报告](../results/vsmt_vm04_target_action_probe_v1.json)由服务器`a46fcec`独立导出，36个原槽完整、0新episode/训练/正例；20个物理干预槽中16个可见性生命周期动作到达探针终态、RELINK 3个位置吻合但碰撞未查、1个在动作返回成功时就少移动约4.98 cm。失败原槽不重选、不重跑；具体放置/碰撞原因没有证据，不能用训练识别器来解释。v1资源benchmark的`RUSAGE_SELF`只计Python worker，D-169勘误保持原v1字节；新[v2关闭式合同](../configs/vsmt/vm04_target_action_probe_proposal_v2.json)在本地加入运行期间cgroup采样/4倍观测需求门，45项纯检查通过，但执行位仍false，未运行v2服务器动作。下一步用户审v2代码及采样限制；生成/训练继续关闭。

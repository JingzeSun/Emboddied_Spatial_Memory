...
**当前D-119正以显式EGL后端重开E0-v2的r4-36固定33相位检查。** `278ae70` check为45/45；首目录已封存264项公开输出，但在首个私有renderer前因shell缺`MUJOCO_GL`停止并保留。新绑定只固定`MUJOCO_GL/PYOPENGL_PLATFORM=egl`并改用独立目录，E0-v2及通过门不变。全开发/验证、双M、数据重建、三模型拟合和确认均未授权，E0-v1、LOG-133与原失败目录保持只读。

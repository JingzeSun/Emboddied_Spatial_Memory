...
当前执行点：VM-04.E 全部完成，E-08 报告已导出并由 [LOG-206](../EXECUTE.md) 收口。D-221 最终得到2,121个成功house、55个失败house、67,872帧公开RGB-D与396维特征；E-06结构单头完成训练，但E-08显示它不适合作P08 bottleneck硬门（整体accuracy 0.7013，bottleneck recall区间0.043–0.150）。用户已批准D-223修订稿：该头完全退出production，P08改由生成器专用可达图上的原冻结拓扑规则认证。F-00实现与本地测试已经完成，当前等待用户代码审查；真实两房服务器预检与所有下游执行授权仍为false，审查后另行授权。P04/P08正式资格、route/raw和private evaluation仍关闭。
...
| 项 | 内容 |
|---|---|
| 状态 | 实现与本地测试完成，等待用户代码审查；真实两房服务器预检待另行授权 |
...
| D-223 文档与合同修订 | 已批准 | F-00实现候选已完成；真实服务器执行仍关闭 |
...
1. 用户审查F-00实现候选及其8项定向测试；真实服务器执行仍关闭。
2. 审查通过后另行裁决是否授权两房服务器真实预检；运行前核对并发资源。
3. 若真实预检存在合格路线再实现F-01；若不存在则按第八章暂停并触发场景设计裁决。

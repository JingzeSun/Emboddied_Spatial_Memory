...
**D-147已按协议先行完成本地实现：公开边create/bind不再允许调用者抑制模板；无模板骨架邻接改走先验验证的专用入口，非受信任create/bind及受信任非邻接三类失败均保持revision逐字节不变，捕获异常后`finish`仍是真NOOP。**协议/实现/运维绑定提交依次为`0be2854`、`122cea6`、`74161b6`；固定入口本地通过executor 42、L1 31、VSMT 106共179项及同构smoke。仍未推送或连接服务器，D-144工程基线摘要继续为null。

...
当前执行点：**S0 五份合同全部审查通过（LOG-214～LOG-219），S1-01 资产与容量授权申请材料已实现待审**（LOG-220），**D-224-S1 九项裁决已批准并落地，服务器冻结资产已只读核验通过**（LOG-221）。D-224 与 D-224-E/F/G 已批准；旧方向已归档到 `archive/pre-d224-unified-graph`，`main` 只含精简版文档。四项已知冲突中三项已消解（Python 按两解释器分工、Vulkan 可枚举到 NVIDIA GPU、ProcTHOR-10K 定为 0.1.2），**仅 ViT-B/14 未登记仍挡住 S1-03～S1-05**；基础环境缺 hydra 的阻塞已于 LOG-222 解除，`import sam2.build_sam` 成功且 torch 与被钉住的工作树未变。当前开四个位（三只读/本地登记加一依赖安装），没有任何资产下载、数据生成或训练授权；单 worker 实测占用尚未测量，因此 worker 数仍算不出。
...
| 项 | 内容 |
|---|---|
| 状态 | **已实现待审，资产已核验、import 依赖已装**（LOG-220/221/222；本地 73 项只读检查通过，六个 lean 模块加边界测试共 351 项通过。七项冻结资产标识与登记值逐一一致；按 D-224-S1 开四个位——三个只读/本地登记加一个依赖安装，未下载任何资产、未装模拟器） |
| 输入 | D-215 冻结的 SAM 2.1 摘要；LOG-136 来源回执登记的 DINOv2、AI2-THOR、ProcTHOR 标识；ProcTHOR-10K 由 D-224-S1 定为 0.1.2 并按上游 LFS 指针登记；**仅 DINOv2 ViT-B/14 摘要仍未登记** |
...
| 继续门 | 登记在获取之前；标识不全的资产不可获取（当前仅 ViT-B/14）；摘要不符即停且不得换镜像、换版本或先用着；worker 数有实测依据并写进回执——容量读数已有，但单 worker 实测占用尚未测量，因此 worker 数仍不可算 |

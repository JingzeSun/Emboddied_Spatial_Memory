...
**当前D-117已获用户语义批准，E0-v2边界兼容区间实现候选待代码审查。** 新版保持原0.2 mm严格更远量，只允许`B* F+ B*`：边缘B像素扩大墙区间且永不认证自由，中央必须有严格F；无F、较近遮挡、无效或冲突继续硬停止。当前只交独立模块、配置与不含r4-36数值的人工正反例；审查后才可在新目录运行既有33相位扫描，再决定全开发/验证公开评价。双M、数据重建、三模型拟合和确认均未开始，E0-v1、LOG-133与原失败目录保持只读。

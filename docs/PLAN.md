...
**当前D-121已批准E0-v2升级为新开发数据版本前端，固定复用50个完整源家族并只重建中断的`r4-23/r4-58`；代码候选待用户审查和服务器零模拟检查后运行。** 相机继续沿每家族固定x位置作原y轴往返，不新增时变x；旧E0-v1、`r4-36` no-go及所有失败目录只读保留。新入口只形成52家族摘要索引和两个新家族，不读12个confirmation、不运行双M或训练。若完整SH-05后D/F/W三者均满足训练前锁定的同一“表现良好”门，才在下一数据版本加入时变x/y观察；当前分母和结果不得回写。D-120的800项E0证据仍见LOG-136。

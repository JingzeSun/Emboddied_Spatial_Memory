...
**当前为R4-3公共前端与控制接口审议稿已交付（D-085，proposed）。** D-084四家族工程及合同证据已于69a8c92验收：原提交7a2005c、50/19项来源匹配、37/33项检查成功，16历史/144首次/144重放及物理/不可见性/64项E0/固定单帧信息门通过，见LOG-116；无需重跑。本轮按用户“开始做吧”细化v2公共输入、R选帧、对象关联、静态地图、M有限力预测与P机器人条件的权限，未实现新科学代码。下一职责建议先交R4-3a公共反投影/覆盖检索的代码、手算例和必要服务器检查；对象/地图/控制的待定数值先分别审议，不在未审模块上堆叠。其余60家族、确认、学习、下载及合并main仍未开放。旧LOG-113失败与现场保留，两版生成耗时和存储仍按D-082合并计账；已有工程通过不代表模型接入或有效。
...
|---|---|---|
| R4-3a 公共反投影与覆盖检索 | 按METHOD具体规则交无文件读取的公共值函数、人工输入输出及独立服务器check/verify/export；不依赖对象拟合/地图/模型 | 原生80像素与轴向深度、裁剪、负坐标floor、去重、2+8贪心及并列/零新增、近期/前缀隔离、原时间/像素来源。用户审过该代码及同版必要回执后继续，不以选中10帧推定恢复两门 |
| R4-3b 当前对象公开关联 | 先固定几何拟合、支持/残差/歧义、两帧速度、自旋/倾斜假设和误差传播数值，再交独立实现 | 不依赖真值mask/颜色/初始模板；多候选/遮挡/单帧返回未决；支持可追到像素。只做已定人工检查，实际16历史验证须登记清单/预算 |
...
本轮无新测试/运行命令、下载或计算预算。a交付时固定其标准库测试清单及资源；后续实际恢复/近似动力学耗时记入另审工程预算，不记成原模拟重新执行，亦不把D-079待审总CPU额度当已授权余额。R4-4还需适配80像素的编码器输出（原四层stride2在80输入下不是原4×4）、九候选曝光和PointWorld资产；R4-3不私自决定resize/pooling或扩大训练预算。剩余非确认家族的生成放行仍须另核原清单、资源和依赖，不因R4-3a通过自动展开。

...
> 版本：v0.4（当前仅冻结 Q1，已纳入 D-008 方向迁移审计）
> 状态：Q1 已由用户确认；Q2–Q6 等待用户逐项回答  
> 定位：这是整个具身智能项目中的一个方法模块，不是整个项目计划

> 方向整合说明：D-008 已在概念上把 Q2–Q5 缩窄为“由 structured innovation 检索受影响旧子图、生成版本化 delta、按任务选 ActiveContext、再把稳定变化巩固到长期记忆”，但具体图范围、控制器、读写边界和下游仍未由用户冻结。统一优先级、技术流和人工确认项见 `09_integrated_direction_plan.md`。

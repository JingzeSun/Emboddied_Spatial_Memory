"""R4-3a server-only artificial coverage checks; run/verify/export once.
...
STAGE = "SH-04-R4-3a-coverage-v1"
RUN = Path("/root/autodl-tmp/spatial-history/sh04-r4-3a-coverage-v1")
REPORT = ROOT / "results/spatial_history_r4_coverage_v1.json"
CONFIG = "configs/spatial_history/r4_coverage_check_v1.json"
TESTS = ("tests/spatial_world_model/test_r4_coverage.py",
         "tests/spatial_world_model/test_r4_coverage_ops.py")
BOUND = ("src/spatial_world_model/__init__.py", "src/spatial_world_model/pair_contract.py",
         "src/spatial_world_model/r4_coverage.py", *TESTS,
         "ops/spatial_history/contract_check.py", "ops/spatial_history/r4_coverage_check.py",
...
CLAIMS = {"real_history_retrieval_verified": False, "public_geometry_recovery_verified": False,
...
def configuration():
    ...
    expected = {"version": "sh04-r4-coverage-check-v1", "decision": "D-086",
                "scope": "server_only_artificial_depth_and_coverage_checks",
                ...
                "claims": CLAIMS,
                "sensor": {"near_depth_m": 0.04, "far_depth_m": 20.0, "clip_margin_m": 0.0001},
                "selection": {"voxel_m": 0.02, "recent_keep": 2, "additional_frames": 8,
                              "origin_m": [0.0, 0.0, 0.0]}}
    require(encode(value) == encode(expected), "coverage config changed")
    shared = read(ROOT / "configs/spatial_history/public_geometry_parallel_v1.json")["public_sensor_spec"]
    require(all(value["sensor"][k] == shared[k] for k in value["sensor"]), "shared sensor changed")
    return value
...
def verify(directory):
    ...
    require(receipt["stage"] == STAGE and receipt["exit_code"] == 0, "no successful coverage receipt")
    ...
    require(receipt["claims"] == CLAIMS and receipt["new_simulation_steps"] == receipt["new_training_steps"]
            == receipt["new_weight_download_bytes"] == 0, "scope/claims changed")
...
def run(directory):
    ...
    try:
        ...
        require(passed, "coverage checks failed; preserve stage and export")
        return verify(directory)
...
def export(directory, report_path):
    configuration()
    require(directory.is_dir(), "no coverage stage to export")
    ...
    value = {"kind": "r4_coverage_artificial_engineering_check", "status": "passed" if error is None else "failed_or_incomplete",
...
def main():
    ...
    def timed_out(signum, frame):
        raise KeyboardInterrupt("coverage command exceeded 300 s; preserve stage")

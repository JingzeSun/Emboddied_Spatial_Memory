...
CONFIG='configs/spatial_history/r4_frontend_stage_v2.json'
...
def parent():
    ...
    value=read(path)
    require(value['status']=='passed' and value['verification_error'] is None,'original engineering evidence')
    return value


def bound_files():
    return sorted(set(parent()['receipt']['binding']) | {CONFIG,
        ...
        'ops/spatial_history/r4_frontend_stage_v2.py','docs/DECISIONS.md'})
...
def check():
    ...
    suite=unittest.defaultTestLoader.loadTestsFromModule(importlib.import_module('test_r4_frontend_v2'))
...
def predict():
    ...
    registration=parent()['public_seal']['source_public_files']; sources={};worlds=[]
...
def evaluate():
    ...
    registration=parent()['summary']['source_truth_files'];sources={};worlds=[]
    for family in CFG['families']:
        for world in CFG['worlds']:
...
def run():
    for command,step in (('check','check'),('predict','public'),('evaluate','evaluation')):
        ...
        code=subprocess.call([sys.executable,'-B',str(Path(__file__).resolve()),command],cwd=ROOT)

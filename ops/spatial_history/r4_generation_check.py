...
from spatial_world_model.r4_families import manifest, validate_manifest, family_config, engineering_rows, categorical_shortcut
from spatial_world_model.r4_generation import save, run_family, verify_family
...
CONFIG = "configs/spatial_history/r4_generation_v1.json"
RUN = Path("/root/autodl-tmp/spatial-history/sh04-r4-engineering-subset-v1")
REPORT = ROOT / "results/spatial_history_r4_engineering_subset_v1.json"
STAGE = "SH-04-R4-2-engineering-subset-v1"
TESTS = ("test_r4_families", "test_r4_storage", "test_r4_generation")
BOUND = (
    CONFIG, "configs/spatial_history/protocol_r4_v1.json", "configs/spatial_history/two_gate_engineering_v1.json",
    ...
    "src/spatial_world_model/two_gate_engineering.py", "src/spatial_world_model/r4_query.py",
    "src/spatial_world_model/r4_scoring.py", "src/spatial_world_model/public_geometry.py",
    "src/spatial_world_model/public_geometry_audit.py",
    *[f"src/spatial_world_model/{name}.py" for name in
      ("r4_families", "r4_trajectory", "r4_physics", "r4_storage", "r4_generation")],
    *[f"tests/spatial_world_model/{name}.py" for name in TESTS],
    "tests/spatial_world_model/test_two_gate_contract.py",
    "ops/spatial_history/r4_generation_check.py", "ops/spatial_history/public_geometry_resources.py",
...
def configuration():
    c = read(ROOT / CONFIG)
    require(c["version"] == "sh04-r4-engineering-subset-v1" and c["tests"] == list(TESTS), "wrong stage config")
...
def guarded_command(seconds):
    ...
    def exceeded(signum, frame):
        raise RuntimeError(errors[0] if errors else "resource guard interruption")
...
def check():
    ...
    design = read(ROOT / "configs/spatial_history/r4_family_design_v1.json")
...
def run(reviewed_code, quota_gib, workers):
    ...
    try:
        ...
        summary = {"families": [{"family_id": r["family_id"], "result": result} for r, result in zip(rows, results)],
                   "accepted": all(r["accepted"] for r in results),
                   "categorical_shortcut": categorical_shortcut([r["success_matrix"] for r in results]),
                   "remaining_storage": {"remaining_families": 60,
                       "reserved_worst_case_bytes": 60 * c["limits_proposed"]["family_bytes"],
                       "data_and_features_total_limit_bytes": 32 * 1024**3,
                       "all_raw_family_hard_bound_bytes": 64 * c["limits_proposed"]["family_bytes"],
                       "features_and_metadata_reserve_bytes": 8 * 1024**3,
                       "allocation_within_32gib": 64 * c["limits_proposed"]["family_bytes"] + 8 * 1024**3 <= 32 * 1024**3,
                       "remaining_inventory_storage_verified": False,
                       "reason": "hard allocation fits; full remaining physical data and rental quota are not yet verified"},
        ...
        summary["categorical_shortcut"]["valid_physical_evidence"] = all(
            r["checks"]["physics"] and r["checks"]["replay_equal"] for r in results)
...
def verify():
    ...
    for row in engineering_rows(read(RUN / "design.json")):
        verify_family(RUN / "execution" / row["family_id"] / "data")
...
def export():
    ...
    for p in RUN.rglob("*.json"):
        if p.name in ("summary.json", "failure.json", "release.json", "processes.json", "exit.json", "started.json",
                      "public_manifest.json", "labels_manifest.json", "audit_manifest.json"):
    ...
    from spatial_world_model.r4_families import encode

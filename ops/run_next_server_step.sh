#!/usr/bin/env bash
# Unique active CPMT server phase: regenerate 12-group v8 health and prove that
# the D-043 training-only contract did not change scientific arrays.
#
# Prerequisites:
...
#
# Read boundary: tracked train generator/config/source, Git metadata and the
# matching full-test marker.
# Write boundary: one ignored outputs directory containing 12-group train-only
# arrays and manifest.
# Resume policy: reuse only a complete arrays/manifest pair that passes every
# check below. A partial pair is never overwritten or deleted. This stage does
# not rerun tests, train, run causal evaluation, read validation/test, export
# results, or modify Git.

set -uo pipefail

CPMT_SERVER_STEP_ID="m1_v6_v8_d043_health_g12_arrays_digest_invariance"
...
CPMT_EXPECTED_ARRAYS_DIGEST="e924f96d4cf28179df010766e4275244cbb78cb9f9425ed514093bdbca3958c3"
...
printf "read_boundary=train_generator_config_source_git_and_full_test_marker\n"
...
printf "expected_arrays_digest=%s\n" "$CPMT_EXPECTED_ARRAYS_DIGEST"
...
if [[ -f "$CPMT_HEALTH_ARRAYS" && -f "$CPMT_HEALTH_MANIFEST" ]]; then
  ...
  mkdir -p "$CPMT_HEALTH_DIR" || cpmt_fail "cannot_create_health_directory"
  (
    cd "$CPMT_REPO_DIR" || exit 2
...
python - \
  ...
  cpmt_fail "health_or_arrays_digest_validation_failed"
import json
import sys
from pathlib import Path
...
printf "NEXT=review_health_then_rewrite_entry_for_1000_group_train_generation\n"

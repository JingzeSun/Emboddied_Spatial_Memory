#!/usr/bin/env bash
# Unique phase: full tests for the D-050 independent S5 confirmation entry.
# Inputs: clean checkout, accepted training export, frozen S5 confirmation plan.
...
set -uo pipefail
CPMT_SERVER_STEP_ID="m1_v6_d050_s5_confirmation_full_test"
CPMT_EXPECTED_TESTS=250
...
CPMT_OUTPUT_DIR="/root/autodl-tmp/cpmt_outputs/m1-v6-d050-full-test-$CPMT_COMMIT_SHORT"
[[ "$#" -eq 0 ]] || exit 2
...
STAGE='m1_v6_d050_s5_confirmation_full_test'
...
def main():
    ...
    print('PLAN_BOUND saved_models=60 confirmation_groups=200 validation_data_read=false test_access=false',flush=True)
    marker_path=out/'full_test.ok.json'
    if marker_path.exists():
        marker=read_json(marker_path);validate_confirmation_test_marker(marker,root,plan)
        ...
        marker={'schema_version':'cpmt-s5-confirmation-tests-v1','commit':provenance['git_commit'],
                'plan_sha256':protocol_sha256(plan),'registration_sha256':plan['post_probe_registration_sha256'],
                ...
                'source_tree_unchanged':before==after,'test_access':False,'validation_data_read':False,
    ...
    print('NEXT=deliver_frozen_200_group_validation_generation_as_a_separate_stage',flush=True)

#!/usr/bin/env bash
# Unique active CPMT server phase: full test of the accepted D-044 metric layer.
#
...
CPMT_SERVER_STEP_ID="m1_v6_v8_d044_evaluation_protocol_full_test"
...
CPMT_OUTPUT_DIR="$CPMT_REPO_DIR/outputs/m1-v6-v8-d044-full-test-$CPMT_COMMIT_SHORT"
...
PY
  ...
  [[ "$CPMT_TEST_COUNT" -ge 202 ]] || cpmt_fail "unexpected_test_count"
  python - "$CPMT_TEST_MARKER" "$CPMT_CURRENT_COMMIT" \
    "$CPMT_EXPECTED_PROTOCOL" "$CPMT_EXPECTED_DATASET" \
...
payload = {
    ...
    "stage": "m1_v6_v8_d044_evaluation_protocol_full_test",
    "commit": sys.argv[2],
...
printf "NEXT=rewrite_entry_for_fixed_train_only_endpoint_probe_after_review\n"

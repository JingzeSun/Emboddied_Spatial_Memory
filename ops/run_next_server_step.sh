#!/usr/bin/env bash
# Unique active CPMT server phase: full-test the planning-only runtime profiler.
#
# Prerequisites:
# - origin/main and the clean server checkout point to this handoff commit;
# - the history contains the successful 1,000-group generation handoff;
# - no generated arrays, validation data or test data are read.
#
...
# match this checkout. This stage does not generate data, train, profile a
# model, run causal evaluation, read validation/test, export results, or
# modify Git.

set -uo pipefail

CPMT_SERVER_STEP_ID="m1_v6_v8_d043_runtime_profile_full_test"
CPMT_PREREQUISITE_COMMIT="8a837e65ea8279a460ed712654142da888956af4"
...
CPMT_RUN_DIR="$CPMT_REPO_DIR/outputs/m1-v6-v8-d043-profile-full-test-$CPMT_SHORT_COMMIT"
...
git -C "$CPMT_REPO_DIR" merge-base --is-ancestor \
  ...
  cpmt_fail "train_generation_handoff_not_in_history"
...
printf "generation=false training=false profiling=false validation_access=false test_access=false\n"
...
PY
  ...
  printf "NEXT=rewrite_entry_for_two_architecture_runtime_profile\n"
  exit 0
fi

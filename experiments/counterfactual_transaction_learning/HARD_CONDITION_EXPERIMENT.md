...
## 新观测 latent 与旧记忆的影响边界

本节是用户于 2026-09-11 确认的任务语义澄清，沿用既有 observation/world 区分及唯一 CTL 主张；不新增模型、预算、通过门或实验阶段。“新”一律先解释为当前新到达的 observation latent（观测表征），而非已判定的新实体。只有当讨论具体 BIRTH/REPLACE 案例并明确其世界真值时，才使用“新物体”指身份新增。

白话：要解决的是“这一眼得到的新表征，应怎样影响此前记住的世界”。输入为当前区域观测 latent、截至当前可得的位姿/可见性/可靠性和旧记忆；输出为候选事务的选择及执行后的新记忆版本。例如椅子的新视角观测可以补充旧身份的证据，也可能揭示其旧位置有误；它不等于每帧都创建新节点，不是先获得目标身份答案再决定更新，也不等于推理时训练视觉 backbone。

### 任务对象与两类变化

| 当前观测支持的解释 | 对旧记忆的合理影响 | 不应由“观测是新的”直接推出的动作 |
|---|---|---|
| 同一物体的另一个视角 | 关联旧身份、按事务补证据，保留仍成立的关系 | 因向量不同就建立新身份或改位置 |
| 同一物体确实移动 | 保留身份，修订失效的位置关系 | 把旧身份当作另一个新对象 |
| 遮挡、噪声或无效位姿/深度 | 按既有证据门及 wrapper 规则保留/暂存，不无依据改旧事实 | 用单次未见就判定旧实体消失 |
| 当前证据支持新身份 | 可考虑 BIRTH；是否关闭旧事实须有另行证据和合法事务 | 只因发现新对象就删除旧档案 |
| 新观测暴露旧档案的混合或重复 | 比较 SPLIT/MERGE 等对身份、证据及其他旧记录的影响 | 把“来自同一观测”直接当合并参数的真值 |

这些是输入语义的例子，不新增或放宽程序前置条件，也不改变某个已登记 family 的参考程序。REPLACE 案例中的“新物体”只描述该场景的身份断裂，不能代表所有新到达的观测。

“对旧 latent 的影响”分为两个不同的验证对象：

1. **结构及引用影响（M1 已有实现）**：修改旧节点身份/生命周期、位置与其他关系、证据和 `latent_refs` 的归属。候选在同一 immutable base 的副本上真实执行，提交后世界和后续连续错误被记录。应正确修订的目标、应保留的目标内容及不相关旧记忆都可能受影响；protected state/合法性检查不代替语义正确性判断。当前 active/open-memory、collateral 及累计错误事实指标只支持各自已定义的结构/事实结论。
2. **canonical 向量数值影响（planned，当前未验证）**：当前观测如何改变旧世界 latent 的向量值，以及这种改变是否损害其他视角/重访观测的解释能力。当前 M1 没有学习该更新算子，不用图 exact、latent 引用合并或固定外观代理的变化冒充它已通过。将来若研究这一层，须先定义版本化向量状态、允许的固定/轻量更新机制和可观察的检验量；向量差为零不自动意味着正确，差越小也不自动越好。本节没有接受新的向量损失、EMA、端到端 backbone 或扩模型实验。

白话：两层区分解决“我们到底改了档案还是改了档案里的向量”的混淆。输入是旧状态、同一当前观测和执行后的状态；输出是可归因的修改记录。例如 MERGE 合并 `latent_refs` 列表属于第一层，把两个 latent 数值融合成更能解释多视角观测的 canonical 向量属于第二层；前者成功不等于后者已经实现。这里不新增 co-primary 或验收阈值。

### 当前实现中的输入和状态对应

| 代码字段/函数 | 当前作用与来源 | 可以与不可以据此声称的内容 |
|---|---|---|
| `current_regions[].anonymous_signature` / `_observed_appearance` | 按合成场景的外观、可见性和观测噪声得到当前匿名外观向量 | 是 M1 对观测 latent 的固定解析代理；不是已接入真实视觉 backbone，也不预先等同新实体 |
| `topology.semantic_latents` / `appearance_of` | 场景生成时的外观表及默认固定描述，供解析观测/匹配使用 | 支持受控合成外观语义；不是从当前观测在线优化出来的旧 canonical memory 向量 |
| `prior_world` 中的节点、边、`evidence_refs`、`latent_refs` | 已有世界记忆及引用，按确定性事务建立新版本 | 能检查结构、证据、引用及历史的影响；引用内容不等于向量数值 |
| `_appearance_match` | 当前匿名外观与旧世界可用实体的固定外观代理作匹配 | 用于 M1 解析感知摘要；不是新实现的 Projective Node Orbit，也不直接提供向量更新能力 |
| `proposal_observation.node_query/edge_query/place_query/merge_queries` | `_event_plan` 目前从参考程序的目标 ID 等信息构造含噪检索提示；生成器据此排序候选，学生读取其中已登记的匹配特征 | 是与匿名观测不同的额外信息通道；不能仅因字段位于 online 或形状像 latent，就认定已由当前合法观测算出 |

### 信息来源判据与本轮未改事项

从当前观测 latent、当前位姿和旧记忆计算 query 本身是允许的研究输入；当前观测能关联哪个旧节点应由这些信息推断。训练 teacher 使用候选实际执行后的世界及随后真实观测，仍符合 CTL；在线学生不能提前读取未来或不可观测的目标身份提示。仿真真值用于生成受声明观测模型约束的当前测量，与把参考程序参数额外编码为提示，不能混为一谈。

现有 query 是否可由上述合法输入重建，需要单独审查；仅在已经生成 query 后删除 `reference_spec`，只能证明下游不直接读该字段。当前角色编码没有新增 merge query 的直接配对分，但这不消除原生成端或三条已有 query 的来源问题。“新”含义澄清不会自动证明该通道合法，也不会把正常的新观测 latent 本身判成答案泄漏。

本节只对齐任务与实现证据：不改科学源码/机器配置、现有训练输入、候选、teacher、损失或已保存结果；不重跑 S5/D-057，不追加 seed，不解封 test。可继续用已有结构评测解释旧记忆怎样被影响；query 来源修正或 canonical 数值更新若要实施，仍需先确定独立、有界且符合首篇范围的方案。S5 的停止结论不会因术语澄清被改成通过。

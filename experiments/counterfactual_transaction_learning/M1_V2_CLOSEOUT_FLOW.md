...
- 当前阶段：**S4 M1-v6：D-043 的服务器 191 项完整测试已通过；下一步只做 12-group v8 arrays digest 不变性复核，通过后才生成 1000-group train arrays**。
...
- 当前分支：D-043 提交 `53539ce` 已在干净服务器运行 191 项测试，327.918 秒全部通过，protocol SHA-256=`73666cabb77b4884302d77ca621669bfdc77e86a44951b8a92b97208509c0eec`，匹配 JSON marker 已落盘。主臂为 Pre-LN＋末端 LayerNorm；两架构的 A–E 网络/输入/mask/参数形状/12 格搜索空间完全相同，各自只按同一 train/inner-dev reference accuracy 选格，同批结果另报共享格和 A/E 交叉格。当前活动入口只重做 12-group train-only health，并要求 arrays digest 逐位等于 D-041 的 `e924f96d…`；不训练、不读取 validation/test。

...
| 固定 anchor probe | 错误分支检查通过后、完整网格前；修正版 F、三项 co-primary 非退化、H3/H1 诊断及六格 SD；按既定规则登记 N | 已交付新入口，服务器待运行：prepare 重建并逐数组核验固定 201 组、检查 F 后，才可 train/evaluate；summarize 不允许 graded 切换，机械填写封存登记。不是完整选参 |
| 并行预算与泛化诊断 | 完整网格前；独立进程随机流/汇总等价、资源检查、同一 checkpoint 的 fit 与 inner-dev 同口径分数和差距；并行交付同时覆盖选参和 S5 全 train 重训 | 公共 worker 已支持预算分区/refit 全 train 两种模式，固定十组 GPU 1/4 进程对拍已交付但未运行（LOG-084）；正式预算与 S5 登记消费、1000 组实际容量仍 planned。优先 GPU 4 进程×CPU 1 线程；差距只作诊断，不增加选参规则 |

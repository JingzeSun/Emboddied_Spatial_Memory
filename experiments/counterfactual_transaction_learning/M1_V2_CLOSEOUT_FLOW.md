...
- 当前阶段：**S4 M1-v6：唯一 1000-group v8 train arrays 已生成并通过健康门；下一步先全测并执行固定 300-step 的两架构运行时剖析，再决定付费服务器上的完整预算网格排程**。
...
- 当前分支：D-043 提交 `53539ce` 已在干净服务器运行 191 项测试，327.918 秒全部通过；12-group train-only health 亦以完全相同的历史 digest 通过。1000-group train 随后用 16 workers 在 1001.0 秒内生成 40,000 rows，teacher agreement=`1.0`、health gate PASS，arrays digest=`e8a890f1b254a7109af641fea57fcbea5efd931b4272d8e96cb870f51604b168`。这份数组是两种架构和 A–E 的唯一共同输入，不再生成第二份。因完整预算共约 180 万次 optimizer updates，先运行不参与选择的固定 300-step 纯成本剖析；不读取 validation/test。

...
- 当前阶段：**S4 M1-v6：D-043 的服务器 191 项完整测试与 12-group v8 arrays digest 不变性复核均已通过；下一步生成唯一 1000-group train arrays**。
...
- 当前分支：D-043 提交 `53539ce` 已在干净服务器运行 191 项测试，327.918 秒全部通过。其后在仅改文档/运维入口的提交上重生成 12-group train-only health：17.957 秒、480 rows（456 online＋24 recovery）、teacher agreement=`1.0`、now pattern 匹配、health gate PASS，arrays digest 逐位等于 D-041 的 `e924f96d4cf28179df010766e4275244cbb78cb9f9425ed514093bdbca3958c3`。这确认 D-043 只改训练合同/架构，没有耦合数据内容。当前活动入口只生成 1000-group train arrays；不训练、不读取 validation/test。

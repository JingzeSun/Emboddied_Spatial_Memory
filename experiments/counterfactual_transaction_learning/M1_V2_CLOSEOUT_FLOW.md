...
- 当前阶段：**S4 M1-v6：MLP 训练后产物验收的旧方法键名断言已修正；当前唯一入口在独立 acceptance worktree 复验两臂已有预算报告（EXECUTE LOG-057）。持锁的主架构仅显示 pending，不重启；原训练 checkout 与报告保持不变。两臂验收通过后再交付独立导出阶段，不访问 validation/test。**

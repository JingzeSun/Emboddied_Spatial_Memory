...
- 当前阶段：**已收到修正版本完整测试成功回执（EXECUTE LOG-073）；当前已交付入口为 v7/v9 的 1000-group train 生成与验收，复用匹配的全测 marker。生成通过后先做 D-051 固定 train 错误分支工程检查，通过后再进入固定 probe 与预算；按 D-052 一次准备已确定阶段的脚本、同步一次、按固定命令顺序执行，不再逐步改写唯一入口。D-051 的预算重跑、固定 endpoint、N 下限与防过拟合规则保持，4 进程×1 线程仍仅为后续预算调度初选；新 probe/registration 与完整预算并行接线尚待完成。不恢复旧 S5、不启动新预算或 test。**
...
| S4 | 正式 run 的分母、能量、架构和终止规则是否唯一 | 按 D-040 固定总混合 group 与真实 C10/C11；按 D-041 固定能量和机制审计；D-043/D-046 冻结 Pre-LN 主臂、A–E 相同 12 格、C 固定计算格后的两条附加权重路径及逐方法/共享/交叉预算读数；D-044–D-047 在网格前以固定 train-only anchor 机械选择 exact 或同构 graded endpoint、以前瞻冻结的 AUC `40/80` 确定三类 co-primary 的 test N，并固定 `(0,0)` 无选择 gate、全过程 open-fact burden、节点与完整 collateral 安全量及 H3-vs-H1 teacher 主文机制对照 | D-043 服务器全测、12-group digest、1000-group train arrays 与 4.367 小时历史成本剖析均已通过；D-046 顺序权重的保守总投影约 5.036 小时；D-044–D-047 本地接线完成，待科学代码服务器全测及一次 endpoint probe；架构间不择优，endpoint 开关不读赢家/方向/效应，H=1 不参与选择或成败门，validation/test 仍封存 |

...
- 当前阶段：**S4 M1-v6：D-046 已在任何 endpoint probe/validation/test 结果前接受；AUC `40/80` 持续等价门、C train/inner-dev 顺序权重选择、纯 validation confirmation 与 attempt/commit/quarantine 报告已本地接线。下一步只在干净服务器运行 D-044–D-046 full test。成功前不运行 endpoint probe 或完整预算网格；成功后另一次提交才把唯一服务器入口改写为固定 `(0,0)` gate、固定 C weight=1 anchor 的 train-only endpoint probe。**
...
S3 更大 train 规模交互确认（未触发，跳过）
  ↓
S4 正式规模/能量/分母预冻结审计
  ↓ D-044 固定 anchor 先验收 causal endpoint 可用性与 test 检验力，再以 v8 train/inner-dev 跑完整预算网格
  ↓
...
| S4 | 正式 run 的分母、能量、架构和终止规则是否唯一 | 按 D-040 固定总混合 group 与真实 C10/C11；按 D-041 固定能量和机制审计；D-043/D-046 冻结 Pre-LN 主臂、A–E 相同 12 格、C 固定计算格后的两条附加权重路径及逐方法/共享/交叉预算读数；D-044–D-046 在网格前以固定 train-only anchor 机械选择 exact 或同构 graded endpoint、以前瞻冻结的 AUC `40/80` 确定三类 co-primary 的 test N，并固定 `(0,0)` 无选择 gate、全过程 open-fact burden、节点与完整 collateral 安全量 | D-043 服务器全测、12-group digest、1000-group train arrays 与 4.367 小时历史成本剖析均已通过；D-046 顺序权重的保守总投影约 5.036 小时；D-044–D-046 本地接线完成，待科学代码服务器全测及一次 endpoint probe；架构间不择优，endpoint 开关不读赢家/方向/效应，validation/test 仍封存 |
...
- D-044 probe 最终选定的 exact/graded active-world semantic correctness 差值的 95% CI 下界 `>= +0.03`；

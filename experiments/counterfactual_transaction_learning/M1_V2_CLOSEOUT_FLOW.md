...
- 当前阶段：**D-051 已在修正数据/probe 前固定重建、完整原预算重跑、endpoint/N 与防选参过拟合规则。当前唯一入口为旧 train 数组上的有界 1/2/4-process runtime probe（EXECUTE LOG-070），用于选择工程并发设置；不运行新预算搜索或恢复 S5。报告返回后进入独立生产一跳修复、正确性检查与新数据版本接线；生产修复及完整预算分片 runner 尚未实施。**
